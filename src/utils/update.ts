...
export function resolveProfilePluginManifest(profile = DEFAULT_PROFILE, env = process.env, { resolveFn }: any = {}) {
  ...
  try {
    if (!resolveFn && !existsSync(manifest)) return undefined
    ...
    if (dirname(actual).split(sep).at(-1) !== PLUGIN_NAME) return undefined
    return found
  } catch {
    return undefined
  }
 ...
 */
export function installedPackageDirs(profile = DEFAULT_PROFILE, env = process.env, { readFileFn, realpathFn, resolveFn }: any = {}) {
  const manifests = [
  ...
  for (const manifest of manifests) {
    if (!readPackageVersion(manifest, { readFileFn })) continue
    ...
    if (actual.split(sep).at(-1) !== PLUGIN_NAME) continue
    dirs.set(actual, dir)
  }
  return [...dirs.values()]
}
...
function extractArchive(spawnFn, archive, dest) {
  return new Promise<void>((resolve, reject) => {
    ...
    child.once('close', (code) => {
      if (code === 0) resolve()
      else reject(new Error(`tar exited ${code}${stderr.trim() ? `: ${stderr.trim()}` : ''}`))
    })
  })
}
...
async function extractedRoot(readdirFn, dest) {
  ...
  if (names.length !== 1) throw new Error(`tarball unpacked ${names.length} roots`)
...
async function swapPackageDirs(dirs, src, version, { renameFn, rmFn, cpFn, readFileFn }) {
  ...
  try {
    for (const dir of dirs) {
      ...
      if (existsSync(bak)) throw new Error('Previous update backup still exists: ' + bak)
    ...
    try {
      ...
      if (failed.length) throw new Error('Update failed; restore backups manually: ' + failed.join(', '), { cause: error })
      throw error
    }
    ...
    for (const row of staged) await rmFn(row.bak, { recursive: true, force: true }).catch(() => undefined)
  } finally {
    for (const row of staged) await rmFn(row.next, { recursive: true, force: true }).catch(() => undefined)
  }
}
 ...
 */
async function performInstallRelease({
  tag,
  ...
  const parsed = parseVersion(tag)
  if (!url || !parsed) return { status: 'failed', error: `bad release tag ${tag}` }
  ...
  const work = join(
    dshHome(env), 'profiles', String(profile || DEFAULT_PROFILE), 'node_modules',
    `.${PLUGIN_NAME}-${parsed.raw}-${randomUUID()}.work`,
  )
  try {
...
export function installRelease(options: any = {}) {
