...
function fill(template: string, n: string | number) {
 ...
 */
export function formatRelativeReset(resetAt: number, units: RelativeUnits, now = Date.now()) {
  ...
  if (days >= RELATIVE_STAMP_AFTER_DAYS) return undefined
