...
export const KIRO_CALLBACK_PATHS = Object.freeze(['/', '/oauth/callback', '/signin/callback'])
...
export const KIRO_USAGE_VERSION = '1.0.0'
 ...
 */
export const KIRO_REASONING_CLAUDE = Object.freeze({
  low: 'low',
  medium: 'medium',
  high: 'high',
  max: 'max',
...
export const KIRO_REASONING_CLAUDE_XHIGH = Object.freeze({
...
export const KIRO_MODELS = Object.freeze([
  ...
  kiroModel('claude-sonnet-4', 'Claude Sonnet 4', KIRO_CONTEXT_WINDOW, KIRO_VISION_INPUT, false),
...
export function kiroAccountKind(session: any = {}) {
  ...
  if (method === 'idc') {
    ...
    if (provider === 'Enterprise' || (start && start !== BUILDER_ID_START_URL)) return 'idc'
    return 'builder'
  }
  return 'social'
}
...
export function kiroMethodLabel(methodOrSession) {
  if (methodOrSession && typeof methodOrSession === 'object') {
    ...
    if (kind === 'idc') return 'IdC'
    if (kind === 'entra') return 'Entra'
...
export function kiroManagementHost(region = KIRO_DEFAULT_REGION) {
  return `management.${region || KIRO_DEFAULT_REGION}.kiro.dev`
}
...
export function validateKiroIdpEndpoint(raw) {
  ...
  if (url.protocol !== 'https:') throw new Error('kiro enterprise SSO token endpoint must be https')
...
export function validateKiroRefreshToken(value) {
  ...
  if (!token) throw new Error('kiro refresh token is empty')
...
export function validateKiroApiKey(value) {
  ...
  if (!key) throw new Error('kiro API key is empty')
  if (!key.startsWith('ksk_') || key.length < 12) throw new Error('kiro API key must start with ksk_')
  return key
}
...
export function allocateKiroMachineId(prior) {
  if (typeof prior === 'string') return kiroMachineId({ machineId: prior })
  if (prior && typeof prior === 'object') return kiroMachineId(prior)
...
export function kiroTokenTypeHeader(session) {
  ...
  if (method === 'external_idp') return 'EXTERNAL_IDP'
  return undefined
}
...
export function kiroSocialLoginOption(value) {
...
export function kiroSocialTokenRedirectUri(redirectUri, callback: any = {}) {
  ...
  const rawPath = typeof callback.pathname === 'string' && callback.pathname
    ...
    : (() => {
      try {
        return new URL(String(redirectUri ?? '')).pathname || '/'
      } catch {
        return '/'
      }
    })()
  const path = rawPath.startsWith('/') ? rawPath : `/${rawPath}`
  const loginOption = kiroSocialLoginOption(callback.loginOption ?? callback.login_option)
...
export function kiroSocialFlow() {
  return {
    ...
    listen: { host: 'localhost', ports: [...KIRO_CALLBACK_PORTS] },
    callbackPath: KIRO_CALLBACK_PATH,
    callbackPaths: [...KIRO_CALLBACK_PATHS],
...
export function kiroSession(fields: any = {}) {
  ...
  if (method === 'api_key') {
    if (!accessToken) throw new Error('kiro session needs an API key')
  ...
  if (!draft.account) draft.account = method === 'api_key' ? 'api-key' : kiroMethodLabel(draft)
...
export async function exchangeKiroSocialCode(code, verifier, redirectUri, { fetchFn = fetch, callback, machineId: priorMachineId }: any = {}) {
  ...
  const response = await fetchFn(`${KIRO_AUTH_URL}/oauth/token`, {
    ...
    body: JSON.stringify({
      ...
      redirect_uri: kiroSocialTokenRedirectUri(redirectUri, callback),
    }),
  })
  const body = await readJson(response, 'kiro social token')
  const accessToken = trimmed(body.accessToken ?? body.access_token)
  if (!accessToken) throw new Error('kiro social token exchange returned no access_token')
  return kiroSession({
    accessToken,
...
export async function refreshKiroExternalIdp(session, { fetchFn = fetch } = {}) {
  ...
  if (session.scopes) form.set('scope', session.scopes)
  ...
  if (!accessToken) throw new Error('kiro enterprise SSO refresh returned no access_token')
  return kiroSession({
    ...session,
    accessToken,
