...
**推理墙（3007）：** Start Anthropic URL 是对的（`…/zcode-plan/anthropic` + `/v1/messages`）。Desktop 3.11.2 `zcode.cjs` 对 `isZcodePlanOpenAiCompatibleBaseUrl`（base 以 `/zcode-plan` 结尾）注入 **`x-aliyun-captcha-verify-param`**。没有这颗头，网关 `400 code 3007 captcha verify failed`。JWT + UA / `X-ZCode-*` 过不了。本 hop **不**伪造、不接 captcha SDK。导入 / 路由 / Flash Free 目录留下。Harness 试用对话先堵住，直到付费 Coding Plan，或以后做 captcha-bridge。试用请在 ZCode.app 里聊。
...
- 不要伪造 `x-aliyun-captcha-verify-param`，本 PR 不接阿里云 captcha SDK。3007 不是指纹缺口；Start URL 是对的。
