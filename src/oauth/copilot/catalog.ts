...
import {
  COPILOT_API_VERSION,
  COPILOT_DEFAULT_CONTEXT,
  COPILOT_DEFAULT_MAX_TOKENS,
  COPILOT_INPUT,
  COPILOT_MODELS,
  COPILOT_REASONING,
  COPILOT_VISION_INPUT,
...
function supportsVision(row) {
  ...
  if (caps?.supports?.vision === true) return true
...
export function copilotReasoningEffortsOf(row) {
  ...
  for (const wire of allowed) {
    if (wire === 'off' || wire === 'none') mapped.off = wire
    else if (wire === 'minimal' && mapped.minimal === undefined) mapped.minimal = wire
    else if (wire === 'xhigh' && mapped.xhigh === undefined) mapped.xhigh = wire
    else if (wire === 'max' && mapped.max === undefined) mapped.max = wire
  }
...
export function toCopilotPickerModels(payload) {
  ...
  models.sort((left, right) => {
    ...
    if (li !== -1 || ri !== -1) return (li === -1 ? 999 : li) - (ri === -1 ? 999 : ri)
