...
export function parseCopilotApiKey(value) {
  ...
  if (!key || key.length < 8) throw new Error('copilot token is empty')
  return key
}
...
export function copilotSession({
  ...
  if (!refresh) throw new Error('copilot token endpoint returned no refresh token')
...
export async function exchangeCopilotToken(githubToken, { fetchFn = fetch, signal } = {}) {
  ...
  if (response.ok) {
    ...
    if (!parsed) throw new Error('copilot token exchange returned no token')
    return parsed
  }
...
async function refreshGithubToken(session, fetchFn) {
  const refresh = trimmed(session?.githubRefreshToken) ?? (
    trimmed(session?.refreshToken) && !isGithubUserToken(session.refreshToken)
      ? trimmed(session.refreshToken)
      : undefined
  )
  if (!refresh || refresh === session?.githubToken) return undefined
  ...
  if (!response.ok) throw await oauthError(response, 'copilot')
  const payload = await response.json()
  const access = trimmed(payload.access_token)
  if (!access) throw new Error('copilot GitHub refresh returned no access token')
  return {
    githubToken: access,
...
export async function refreshCopilot(session, fetchFn = fetch) {
  ...
  if (!github) {
    ...
    if (!rotated) throw new Error('copilot session needs a GitHub token')
...
export function copilotUpstreamHeaders(session, cacheSessionId, extra = {}) {
  ...
  if (extra.vision) headers['copilot-vision-request'] = 'true'
  return headers
}
