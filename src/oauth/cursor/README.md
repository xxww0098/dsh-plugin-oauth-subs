...
静态 fallback（离线 / RPC 空）对齐 [cursor.com/docs/models-and-pricing](https://cursor.com/docs/models-and-pricing)（2026-09-23 复抓：HTML 表 + `/docs/models/<slug>` Model ID 页 + `llms.txt` 模型目录）：`composer-2.5`、`grok-4.7`、`grok-4.6`、`grok-4.5`、`claude-fable-5-1`、`claude-opus-5-5`、`claude-opus-5`、`claude-sonnet-5`、`gemini-3.1-pro`、`gemini-3.8-flash`、`muse-spark-1.3`、`gpt-5.6-sol` / `terra` / `luna`、`gpt-5.5`。2026-09-23 新增两行：`claude-opus-5-5`（Model ID 页 `/docs/models/claude-opus-5-5`：300k 窗口、页内「replaces Opus 5」；maxTokens 128k 按 `inferCursorMaxOutputTokens` 的 claude 规则）与 `muse-spark-1.3`（Model ID 页 `/docs/models/muse-spark-1-3`：300k 窗口、Meta；maxTokens 64k 家族默认推断；input 走静态行默认 `text`+`image`，docs 无逐模型 vision 标记；参数样式已由 2026-09-26 活目录确认）。同日 `claude-opus-5`、`gpt-5.5` 被 docs 表降为 **Hidden by default**（`llms.txt` 目录不再列）但仍各有官方 Model ID 页；hidden ≠ 订阅后端不服务（`glm-5.2` / `kimi-k3` 同样 docs-hidden，2026-09-23 活目录仍在），区域锁家族保留在静态楼。`grok-4.7` 静态行取 256k 非 Max Mode 窗口与 text-only；500k 是独立 Max Mode 变体，普通 Run 不发送 `maxMode`，官方 docs 页当时仍只有 4.6/4.5（`/docs/models/grok-4-7` 404）。id 抄官方 Model ID（Fable 是 `claude-fable-5-1`，不是点号）。登录、本机导入、额度刷新、以及**启动 warmup**（`controller.warmCatalogs`，只跑已登录家族）走活发现：
