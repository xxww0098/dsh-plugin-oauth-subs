...
export function readVarint(buf, offset = 0) {
  ...
  while (i < buf.length) {
    const b = buf[i++]
    n += (b & 0x7f) * (2 ** shift)
    if ((b & 0x80) === 0) return { value: n, offset: i }
    shift += 7
    if (shift > 63) break
  }
...
export function encodeProtoValue(value) {
  ...
  if (typeof value === 'boolean') return encodeBool(4, value)
...
export function encodeUserMessage({ text, messageId, selectedContextBlob, mode = 1 }) {
  ...
  if (messageId) parts.push(encodeString(17, messageId))
...
export function encodeConversationState({
  ...
  if (clientName) parts.push(encodeString(22, clientName))
  return Buffer.concat(parts)
}
...
export function encodeConversationTurn({ userMessageBlob, stepBlobs = [], requestId }) {
  ...
  if (requestId) agent.push(encodeString(3, requestId))
...
export function encodeAgentRunRequest({
  ...
  if (mcpTools?.length) parts.push(encodeMessage(4, encodeMcpTools(mcpTools)))
  if (conversationId) parts.push(encodeString(5, conversationId))
  if (requestedModel) parts.push(encodeMessage(9, requestedModel))
  return Buffer.concat(parts)
}
 ...
 */
export function encodeTurnEndedUpdate({
  inputTokens,
  outputTokens,
  cacheReadTokens,
  cacheWriteTokens,
  ...
  if (cacheWriteTokens != null) parts.push(encodeUint32(4, cacheWriteTokens))
  if (reasoningTokens != null) parts.push(encodeUint32(5, reasoningTokens))
  return Buffer.concat(parts)
}
...
export function decodeAgentClientMessage(buf) {
  ...
  if (!runRaw) return { conversationId: undefined, modelId: undefined, userText: undefined, tools: [] }
