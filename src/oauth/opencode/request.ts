...
function wireEffort(value, efforts) {
  ...
  if (OFF.has(value)) return Object.hasOwn(efforts, 'off') ? efforts.off : undefined
...
function chatToolToResponses(tool) {
  ...
  if (tool.type === 'function' && isPlain(tool.function)) {
    ...
    if (tool.function.strict != null) next.strict = tool.function.strict
    return next
  }
  return tool
}
...
export function chatToOpencodeResponses(payload = {}) {
  ...
  if (payload.input != null) next.input = payload.input
  else if (Array.isArray(payload.messages)) next.input = messagesToInput(payload.messages)
  else if (typeof payload.messages === 'string') next.input = payload.messages

  if (typeof payload.instructions === 'string') next.instructions = payload.instructions

  const max = firstNumber(payload.max_output_tokens, payload.max_tokens)
  if (max !== undefined) next.max_output_tokens = Math.max(OPENCODE_MIN_OUTPUT_TOKENS, max)
  ...
  if (payload.tool_choice != null) next.tool_choice = payload.tool_choice
  if (payload.stream === true) next.stream = true
...
function partsText(parts, types) {
  ...
  return parts.map((part) => {
    if (typeof part === 'string') return part
    if (!isPlain(part)) return ''
    if (allowed.has(part.type) || !part.type) return part.text ?? ''
    return ''
  }).join('')
}
...
function collectResponsesOutput(payload) {
  ...
  for (const item of Array.isArray(payload.output) ? payload.output : []) {
    ...
    if (item.type === 'message' || item.role === 'assistant') {
      ...
      if (chunk && !text.includes(chunk)) text += chunk
...
export function foldOpencodeReasoningContent(payload) {
  ...
  const choices = payload.choices.map((choice) => {
    ...
    if (!isPlain(message)) return choice
...
export function opencodeResponsesToChat(payload = {}, { model, id } = {}) {
  if (isChatCompletion(payload)) return foldOpencodeReasoningContent(payload)
...
function deltaText(event) {
  ...
  if (isPlain(event?.delta) && typeof event.delta.text === 'string') return event.delta.text
...
export function createOpencodeResponsesChatStream({ model, id = 'chatcmpl-opencode' } = {}) {
  ...
  return {
    push(event) {
      if (!isPlain(event)) return undefined
