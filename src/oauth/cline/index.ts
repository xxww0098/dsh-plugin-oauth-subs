...
export const CLINE_MODELS = Object.freeze([
  ...
  { id: 'x-ai/grok-4.5', name: 'Grok 4.5', contextWindow: 500_000, maxTokens: 450_000, input: ['text', 'image'] },
  ...
  { id: 'z-ai/glm-5.3-flash', name: 'GLM-5.3-Flash (free)', contextWindow: 1_310_720, maxTokens: 131_072, input: ['text', 'image'] },
...
export function clineBearer(session) {
  ...
  if (!token) throw new Error('cline session needs an access token')
  return token
}

export function clineBalanceUrl(userId) {
  const id = trimmed(userId)
  if (!id) throw new Error('cline account id is required')
...
export async function registerClineTokens(tokens, { fetchFn = fetch, signal, source = 'oauth' } = {}) {
  ...
  if (!response.ok) throw await oauthError(response, 'cline register')
...
export function clineSessionFromAuthData(data, fallback = {}) {
  ...
  if (!refresh) throw new Error('cline token endpoint returned no refresh token')
  const expiresAt = clineExpiresAt(data?.expiresAt, fallback.expiresAt)
  if (expiresAt === undefined) throw new Error('cline token endpoint returned no usable expiry')
...
export function parseClineUserInfo(payload) {
  ...
  if (!account && !planType && !active) return undefined
  return {
    ...(account ? { account } : {}),
    ...(planType ? { planType } : {}),
