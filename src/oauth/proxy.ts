...
function completionsUsageMapper(family, wire) {
  if (family === 'copilot') return mapCopilotUsage
  if (family === 'kimi') return mapKimiUsage
  if (family === 'cline') return mapClineUsage
  if (family === 'glm' && wire !== 'anthropic') return mapGlmChatUsage
  return undefined
}
...
class SseUsageRewriter {
  ...
  #rewriteFrame(frame) {
    ...
    return lines.map((line) => {
      ...
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed) || parsed.usage == null) return line
      const usage = this.mapUsage(parsed.usage)
      if (usage === parsed.usage) return line
...
function forwardedHeaders(upstreamHeaders) {
  ...
  upstreamHeaders.forEach((value, key) => {
    ...
    if (HOP_BY_HOP.has(name) || WIRE_ONLY.has(name)) return
    headers[key] = value
  })
  return headers
}
