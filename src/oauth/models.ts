...
function isLegacyOpencodeGoBuiltin(existing) {
  if (existing == null || typeof existing !== 'object') return false
  if (existing.apiKeyEnv !== OPENCODE_GO_API_KEY_ENV) return false
  if (Object.keys(existing).length !== 2) return false
  if (existing.headers == null || typeof existing.headers !== 'object') return false
...
async function syncCompactionPolicies(patchPath, providers) {
  ...
  try {
    ...
    if (error?.code !== 'ENOENT') return { status: 'unreadable', error: error instanceof Error ? error.message : String(error) }
  }
