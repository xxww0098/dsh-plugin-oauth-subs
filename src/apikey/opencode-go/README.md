...
- 对话 / 部署：用户在 DSH **API Keys** 配 `OPENCODE_API_KEY`。宿主 pi-ai 已有内置 `opencode-go`，`api: openai-responses`，直连 `https://opencode.ai/zen/go/v1`。本插件不写 `oauth-opencode-go` 路由。
...
路由：插件启动 / `sync()` 时若 `llm-pi-ai.providers.opencode-go` 缺失，补 `{ displayName: 'OpenCode Go', apiKeyEnv: 'OPENCODE_API_KEY' }`（`ensureOpencodeGoRoute`）。**不写** `api` / `baseURL` / `models`：pi-ai 内置目录提供 27 个模型（21 completions + 4 responses + 2 anthropic，各自带 api）。已在的同名路由**绝不覆盖**。用户只需在 DSH 存 `OPENCODE_API_KEY`。

## 协议

官方：[Go · API 端点](https://opencode.ai/docs/zh-cn/go/#api-%E7%AB%AF%E7%82%B9)。对话是 Responses：
