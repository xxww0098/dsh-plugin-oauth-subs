...
export function parseOpencodeGoVault(text) {
  ...
  if (!legacy.apiKey && !legacy.cookieHeader && !legacy.workspaceId) return emptyVault()
...
export class OpencodeGoStore {
  ...
  async adoptKey(id, key) {
    ...
    if (!entry || entry.apiKey || !value) return false
    entry.apiKey = value
    await this.#persist()
    return true
  }
  ...
  async save({ id, apiKey, cookie, workspace } = {}) {
    ...
    if (cookie !== undefined) {
      ...
      if (isOpencodeGoCookieMask(raw)) nextCookie = undefined
      else if (!raw.trim()) nextCookie = ''
      else {
  ...
  async switch(id) {
    ...
    if (!this.vault.accounts[key]) throw new Error(`OpenCode Go account ${key} is not signed in`)
  ...
  async remove(id) {
    ...
    if (wasActive) this.vault.activeId = this.#ids()[0]
    await this.#persist()
  ...
  async clear(id, field) {
    ...
    if (!entry) throw new Error(`OpenCode Go account ${key} is not signed in`)
    if (field === 'key') entry.apiKey = ''
    else if (field === 'cookie') entry.cookieHeader = ''
    else if (field === 'workspace') entry.workspaceId = ''
    else {
      entry.apiKey = ''
      entry.cookieHeader = ''
      entry.workspaceId = ''
    ...
    await this.#persist()
    if (!entry.cookieHeader) this.quotas.set(key, { status: 'idle' })
    else await this.#refreshOne(key, this.quotas.get(key)).catch(() => undefined)
    return this.#snapshot()
  }
