...
| OpenCode Go（API key） | [opencode.ai/docs/go](https://opencode.ai/docs/go/) | [stablyai/orca](https://github.com/stablyai/orca)；[steipete/CodexBar](https://github.com/steipete/CodexBar) | 对话走宿主内置 pi-ai `opencode-go`（27 模型，由 DSH 模型设置页自行开启；插件不写该 profile）；插件另写**完整官方 Go 目录**：`OpenCode Go`（openai-completions 27 行）+ `OpenCode Go · Responses`（5 行）；7 个公开 id 网关不服务不进目录；`OPENCODE_API_KEY`；插件那条路由带强制的 `x-opencode-session`（pi-ai 0.85.1 不发会话头）；本插件只读额度——Console 迁移后走 `/console/api/{orgs,go/status,billing/status,user}` + `x-org-id`（CodexBar `OpenCodeGoLegacyFallback` 次序），未迁移账号兜底旧 `/workspace/{id}/go` 刮页 | [`opencode-go/README.md`](../src/apikey/opencode-go/README.md) |
