...
test('parseAntigravityQuotaSummary is Gemini / Claude+GPT weekly and 5-hour bars', () => {
  ...
  assert.deepEqual(parsed.rows.map((row) => row.kind), ['heading', 'primary', 'weekly', 'heading', 'primary', 'weekly'])
  assert.equal(parsed.rows[0].product, 'Gemini Models')
  assert.equal(parsed.rows[1].remainingPercent, 96)
  assert.equal(parsed.rows[1].windowMinutes, 300)
  assert.equal(parsed.rows[2].remainingPercent, 99)
  assert.equal(parsed.rows[2].resetAt, Date.parse('2099-02-08T00:00:00Z'))
...
test('QuotaStore fetches Antigravity model groups from daily hub, not IDE prod', async () => {
  ...
  assert.equal(quota.rows[1].kind, 'primary')
  assert.equal(quota.rows[1].remainingPercent, 96)
  assert.equal(quota.rows[2].kind, 'weekly')
  assert.equal(quota.rows[2].remainingPercent, 99)
