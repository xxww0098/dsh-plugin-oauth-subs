...
test('Fast follows each Codex catalog row service_tiers, never Grok', () => {
  ...
  assert.equal(modelSupportsFastMode('openai/gpt-6-sol'), true)
  assert.equal(modelSupportsFastMode('gpt-6-luna'), true)
  ...
  // retired rows / not served to ChatGPT accounts: 400 upstream
  assert.equal(modelSupportsFastMode('gpt-5.4'), false)
...
test('peelFastSuffix strips host-side -fast even on ineligible bases', () => {
  ...
  assert.deepEqual(peelFastSuffix('gpt-5.4-fast'), { model: 'gpt-5.4', requestedFast: false })
  assert.deepEqual(peelFastSuffix('gpt-6-luna-900k-fast'), { model: 'gpt-6-luna-900k', requestedFast: true })
...
test('applyFastMode injects priority only for eligible Codex -fast', () => {
  ...
  assert.deepEqual(
    applyFastMode({ model: 'gpt-5.4-fast' }),
    { model: 'gpt-5.4' },
  )
