...
test('logged-in Kiro 3/3 persist writes oauth-kiro', async () => {
  const settings = createPiAiSettings()
  const result = await syncHarnessModels({
    settings,
    prefix: 'oauth',
  ...
  assert.deepEqual(stored['oauth-kiro'].models.map((model) => model.id), ['claude-sonnet-4.5', 'claude-sonnet-4-8', 'claude-opus-4.6'])
  assert.deepEqual(result.routes.find((row) => row.provider === 'oauth-kiro').models, ['claude-sonnet-4.5', 'claude-sonnet-4-8', 'claude-opus-4.6'])
})
