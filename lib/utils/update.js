 ...
 */
export function pathWithDefaults(current, platform = process.platform) {
 ...
 */
export function runDshPlugin({ spawnFn = spawn, profile = DEFAULT_PROFILE, args, timeoutMs = PLUGIN_UPDATE_TIMEOUT_MS, env = process.env, execPath = process.execPath, } = {}) {
    ...
    return new Promise((resolve) => {
        ...
        try {
            child = spawnFn(spec.cmd, spawnArgs, {
                env: { ...env, PATH: pathWithDefaults(env?.PATH) },
                ...(cwd ? { cwd } : {}),
 ...
 */
export function runPluginUpdate({ spawnFn = spawn, profile = DEFAULT_PROFILE, timeoutMs = PLUGIN_UPDATE_TIMEOUT_MS, env = process.env, execPath = process.execPath, } = {}) {
...
export async function applyHostUpdate({ spawnFn = spawn, profile = DEFAULT_PROFILE, latest, timeoutMs = PLUGIN_UPDATE_TIMEOUT_MS, env, readFileFn, } = {}) {
    ...
    // 'update' is pnpm update and can no-op or stall on a git spec; a timeout
    // still falls through to the deterministic 'add <repo>#tag' retry. Only a
    // missing dsh binary is unrecoverable.
    if (!first.ok && first.status !== 'failed' && first.status !== 'timeout') {
 ...
 */
export function resolveDshInstall(_platform = process.platform, env = process.env, { realpathFn = realpathSync, readFileFn = readFileSync, existsSyncFn = existsSync } = {}) {
    const candidates = [env.DSH_BIN_PATH, process.argv?.[1]].filter((value) => typeof value === 'string' && Boolean(value));
...
export async function fetchDshLatest({ fetchFn = fetch, spawnFn = spawn, current, platform = process.platform, timeoutMs = 10_000, env, readFileFn, realpathFn, existsSyncFn, } = {}) {
...
export function dshInstallPrefix(packagePath, platform = process.platform) {
    ...
    if (idx < 1 || parts.slice(idx + 1).join('/') !== `${DSH_NPM_PACKAGE}/package.json`)
        return '';
...
export function stampDshHostVersion(clientPath, version, { readFileFn = readFileSync, writeFileFn = writeFileSync } = {}) {
    ...
    if (!raw || !parseVersion(raw))
        return false;
    const text = String(readFileFn(clientPath, 'utf8') ?? '');
    if (!DSH_HOST_VERSION_STAMP_RE.test(text))
        return false;
...
export function scheduleDshWebRestart({ spawnFn = spawn, env = process.env, delaySec = 2, execPath = process.execPath, argv = process.argv, cwd = process.cwd(), platform = process.platform, } = {}) {
...
export async function applyHostDshUpdate({ spawnFn = spawn, targetVersion, timeoutMs = DSH_UPDATE_TIMEOUT_MS, env, readFileFn, realpathFn, existsSyncFn, } = {}) {
    const homeEnv = env ?? process.env;
