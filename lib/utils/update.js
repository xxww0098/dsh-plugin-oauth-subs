...
export function resolveDshInstall(platform = process.platform, env = process.env, { realpathFn = realpathSync, readFileFn = readFileSync } = {}) {
...
export async function fetchDshLatest({ fetchFn = fetch, current, platform = process.platform, timeoutMs = 10_000, env, readFileFn, realpathFn, } = {}) {
...
export async function applyHostDshUpdate({ spawnFn = spawn, targetVersion, timeoutMs = DSH_UPDATE_TIMEOUT_MS, env, readFileFn, realpathFn, } = {}) {
    const homeEnv = env ?? process.env;
