...
export function resolveDshInstall(platform = process.platform, env = process.env, { realpathFn = realpathSync, readFileFn = readFileSync, existsSyncFn = existsSync } = {}) {
    const isWin = platform === 'win32';
    const userHome = homedir();
    const candidates = [];
    if (env.DSH_BIN_PATH)
        candidates.push(env.DSH_BIN_PATH);
    if (typeof process.argv?.[1] === 'string' && process.argv[1])
        candidates.push(process.argv[1]);
    if (typeof env._ === 'string' && env._)
        candidates.push(env._);
    if (isWin) {
        candidates.push(join(userHome, 'AppData', 'Roaming', 'npm', 'dsh.cmd'), join(userHome, 'AppData', 'Roaming', 'npm', 'dsh'));
    }
    else {
        candidates.push(join(userHome, '.local', 'bin', 'dsh'), '/usr/local/bin/dsh', '/opt/homebrew/bin/dsh');
    }
