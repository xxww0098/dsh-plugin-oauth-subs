 ...
 * window: 872K on every GPT-6 row and GPT-5.6 Sol / Terra / Luna. The large
 * window is opt-in via a `-900k` suffix, kept as a stable id even though the
 * real ceiling is per-model. The suffix is stripped before the id goes upstream.
 *
 * GPT-5.5 caps at its default window — no variant.
 */
...
export function peelContextSuffix(modelId) {
    ...
    if (isCodex900kBase(base))
        return { model: base, requestedLarge: true };
    return { model: raw, requestedLarge: false };
}
