...
export function classifyToolError(message) {
    ...
    if (timeout)
        return { cause: TOOL_CAUSES.host_timeout, timeoutMs: Number(timeout[1]) };
    if (CASCADE_ABORT.test(text))
        return { cause: TOOL_CAUSES.cascade_abort, timeoutMs: null };
    if (INVALID_ARGS.test(text))
        return { cause: TOOL_CAUSES.invalid, timeoutMs: null };
...
function transportScanText(value) {
    const text = typeof value === 'string' ? value : (() => { try {
