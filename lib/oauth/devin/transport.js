...
export async function runDevinChat(session, built, { signal, onEvent, fetchFn = fetch } = {}) {
    if (!session?.accessToken)
        throw new DevinTransportError('Devin chat needs a session token', { status: 401 });
    const attempt = (auth) => {
    ...
    if (!response.ok) {
        ...
        const error = new DevinTransportError(`Devin chat failed (HTTP ${response.status})${text ? `: ${text.slice(0, 300)}` : ''}`, { status: response.status });
        if (response.status === 401 || response.status === 403)
            error.permanent = true;
        throw error;
    }
    ...
    for (;;) {
        ...
        for (const item of frames) {
            if (item.end) {
                ...
                if (trailer)
                    throw new DevinTransportError(`Devin chat stream error: ${trailer}`);
                continue;
            }
 ...
 */
export async function forwardDevin(response, { payload, cacheSessionId, stream, session, signal, fetchFn = fetch, runFn = runDevinChat, } = {}) {
    if (!session?.accessToken) {
    ...
    const runRetrying = async (onEvent, hasOutput) => {
        for (let attempt = 0;; attempt += 1) {
            ...
            try {
                ...
                if (signal?.aborted || hasOutput?.() || !devinRetryable(error) || attempt === DEVIN_STREAM_ATTEMPTS - 1)
                    throw error;
            }
        }
    };
