 ...
 */
import { DEVIN_MODELS, devinApiServer, DEVIN_MODELS_PATH, DEVIN_MODEL_DISPLAYS, } from './index.js';
import { devinBasicAuth, devinMetadataBytes } from './request.js';
import { decodeGetCliModelConfigsResponse, decodeUnaryBody, encodeGetCliModelConfigsRequest, } from './proto.js';
...
export function devinLabelVariant(label, familyLabel) {
    ...
    if (base && rest.startsWith(base))
        suffix = rest.slice(base.length).trim();
    if (!suffix)
        return { effort: undefined, thinking: false, mods: [] };
    // Multi-word efforts ("no thinking", "x-high", "extra high") before splitting.
    const direct = EFFORT_WORDS[suffix];
    if (direct !== undefined)
        return { effort: direct, thinking: false, mods: [] };
    ...
    if (effort !== undefined)
        return { effort, thinking: false, mods };
...
export function toDevinPickerModels(configs) {
    ...
    for (const config of Array.isArray(configs) ? configs : []) {
        ...
        if (!family)
            continue; // family-less legacy MODEL_PRIVATE_* rows
    ...
    for (const group of groups.values()) {
        ...
        for (const { config, uid, effort } of group.configs) {
            if (effort !== undefined && variants[effort] === undefined)
                variants[effort] = uid;
            if (effort === undefined) {
            ...
            if (config.isDefaultInFamily || config.familyDefault)
                defaultUid = uid;
        ...
        if (!defaultUid)
            defaultUid = variants.medium ?? variants.high ?? group.configs[0]?.uid;
        if (!defaultUid)
            continue;
