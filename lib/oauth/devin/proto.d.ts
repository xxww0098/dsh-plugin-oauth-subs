...
export declare function fieldBool(fields: any, number: any): boolean;
export declare function encodeDevinMetadata({ apiKey, userJwt, ideName, ideVersion, extensionName, extensionVersion, locale, os, modelDisplays }?: {}): Buffer<ArrayBuffer>;
...
export declare function encodeChatToolCall({ id, name, argumentsJson }?: {}): Buffer<ArrayBuffer>;
export declare function decodeChatToolCall(buf: any): {
    id: any;
    name: any;
    argumentsJson: any;
};
export declare function encodeImageData({ base64Data, mimeType }?: {}): Buffer<ArrayBuffer>;
export declare function encodeChatMessagePrompt({ messageId, source, prompt, toolCalls, toolCallId, toolResultIsError, images, thinking, signature, }?: {
    toolCalls?: any[];
    images?: any[];
}): Buffer<ArrayBuffer>;
export declare function encodeChatToolDefinition({ name, description, jsonSchemaString, strict }?: {}): Buffer<ArrayBuffer>;
export declare function encodeChatToolChoice({ optionName, toolName }?: {}): Buffer<ArrayBuffer>;
...
export declare function encodeCompletionConfiguration({ maxTokens, temperature, topP, stopPatterns, }?: {
    maxTokens?: number;
    temperature?: number;
    topP?: number;
    stopPatterns?: any[];
}): Buffer<ArrayBuffer>;
export declare function encodeGetChatMessageRequest({ metadata, prompt, chatMessagePrompts, chatModelUid, configuration, tools, toolChoice, cascadeId, executionId, plannerMode, requestType, }?: {
...
export declare function connectTrailerError(text: any): string;
