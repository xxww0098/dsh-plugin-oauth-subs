...
import { DEVIN_SOURCE_SYSTEM, DEVIN_SOURCE_TOOL, DEVIN_SOURCE_USER, encodeChatMessagePrompt, encodeCompletionConfiguration, encodeDevinMetadata, DEVIN_STOP_REASON_MAX_TOKENS, DEVIN_STOP_REASON_FUNCTION_CALL, } from './proto.js';
import { DEVIN_IDE_NAME, DEVIN_IDE_VERSION, DEVIN_EXTENSION_NAME, DEVIN_EXTENSION_VERSION, DEVIN_LOCALE, DEVIN_STOP_PATTERNS, } from './index.js';
...
export function devinWireModelId(model, reasoningEffort) {
    ...
    if (effort && row.variants?.[effort])
        return row.variants[effort];
    ...
    if (effort && Object.values(row.variants ?? {}).includes(effort))
        return effort;
...
function stopPatterns(payload) {
    ...
    for (const value of Array.isArray(stop) ? stop : stop == null ? [] : [stop]) {
        ...
        if (text && !DEVIN_STOP_PATTERNS.includes(text))
            extra.push(text);
    }
...
export function devinStopReasonToFinish(reason, hasToolCalls) {
    if (hasToolCalls)
        return 'tool_calls';
    if (reason === DEVIN_STOP_REASON_MAX_TOKENS)
        return 'length';
    if (reason === DEVIN_STOP_REASON_FUNCTION_CALL)
        return 'tool_calls';
    return 'stop';
}
/**
...
export function devinToOpenai(collected, { model, id } = {}) {
    ...
    if (collected?.thinking)
        message.reasoning_content = collected.thinking;
    const body = {
...
export function createDevinOpenaiStream({ model, id } = {}) {
    ...
    function toolIndex(callId) {
        if (!toolIndexes.has(callId))
            toolIndexes.set(callId, nextToolIndex++);
