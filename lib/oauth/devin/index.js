...
export function isDevinOpaqueAccount(value) {
    ...
    if (raw.toLowerCase() === 'devin')
        return true;
    if (/^devin-[A-Za-z0-9_-]{4,}$/i.test(raw))
        return true;
    if (/^devin-team\$[A-Za-z0-9_-]+$/i.test(raw))
        return true;
    if (/^user-[0-9a-f]{16,}$/i.test(raw))
        return true;
    return false;
}
export function pickDevinHumanAccount(...candidates) {
    for (const value of candidates) {
        ...
        if (!next || isDevinOpaqueAccount(next))
            continue;
        return next;
    }
    return undefined;
...
export function parseDevinTokenResponse(value, endpoint = 'Devin token') {
    ...
    if (!token)
        throw new Error(`${endpoint} returned no session token`);
    return { token: normalizeDevinToken(token) };
}
export function devinSession({ accessToken, expiresAt, account, planType, apiServer, source = 'pkce', } = {}) {
    const access = normalizeDevinToken(accessToken);
    if (!access)
        throw new Error('devin session needs a session token');
...
export async function refreshDevin(session, { fetchFn = fetch, statusFn } = {}) {
    ...
    try {
        ...
        if (/401|403|unauthenticated|permission/i.test(next.message))
            next.permanent = true;
        throw next;
    }
}
