...
export function encodeDevinMetadata({ apiKey, userJwt, ideName, ideVersion, extensionName, extensionVersion, locale, os, modelDisplays } = {}) {
    const parts = [];
    if (ideName)
        parts.push(encodeString(1, ideName));
    if (extensionVersion)
        parts.push(encodeString(2, extensionVersion));
    if (apiKey)
        parts.push(encodeString(3, apiKey));
    if (locale)
        parts.push(encodeString(4, locale));
    if (os)
        parts.push(encodeString(5, os));
    if (ideVersion)
        parts.push(encodeString(7, ideVersion));
    if (extensionName)
        parts.push(encodeString(12, extensionName));
    if (userJwt)
        parts.push(encodeString(21, userJwt));
    ...
    if (modelDisplays?.length)
        parts.push(encodeBytes(30, Buffer.from(modelDisplays)));
...
export function encodeChatToolCall({ id, name, argumentsJson } = {}) {
    const parts = [];
    if (id)
        parts.push(encodeString(1, id));
    if (name)
        parts.push(encodeString(2, name));
    if (argumentsJson !== undefined)
        parts.push(encodeString(3, argumentsJson ?? ''));
    return Buffer.concat(parts);
}
...
export function encodeChatMessagePrompt({ messageId, source, prompt, toolCalls = [], toolCallId, toolResultIsError, images = [], thinking, signature, } = {}) {
    const parts = [];
    if (messageId)
        parts.push(encodeString(1, messageId));
    if (source !== undefined)
        parts.push(encodeUint(2, source));
    if (prompt !== undefined)
        parts.push(encodeString(3, prompt ?? ''));
    for (const call of toolCalls)
        parts.push(encodeMessage(6, encodeChatToolCall(call)));
    if (toolCallId)
        parts.push(encodeString(7, toolCallId));
    if (toolResultIsError)
        parts.push(encodeBool(9, true));
    for (const image of images)
        parts.push(encodeMessage(10, encodeImageData(image)));
    if (thinking)
        parts.push(encodeString(11, thinking));
    if (signature)
        parts.push(encodeString(12, signature));
    return Buffer.concat(parts);
}
export function encodeChatToolDefinition({ name, description, jsonSchemaString, strict } = {}) {
    ...
    if (strict)
        parts.push(encodeBool(12, true));
    return Buffer.concat(parts);
}
export function encodeChatToolChoice({ optionName, toolName } = {}) {
    if (toolName)
        return encodeString(2, toolName);
...
export function encodeCompletionConfiguration({ maxTokens = 64_000, temperature = 0.4, topP = 1, stopPatterns = [], } = {}) {
    const parts = [
    ...
    for (const pattern of stopPatterns)
        parts.push(encodeString(9, pattern));
    parts.push(encodeDouble(11, 1)); // fim_eot_prob_threshold
    return Buffer.concat(parts);
}
export function encodeGetChatMessageRequest({ metadata, prompt, chatMessagePrompts = [], chatModelUid, configuration, tools = [], toolChoice, cascadeId, executionId, plannerMode = DEVIN_PLANNER_MODE_DEFAULT, requestType = DEVIN_REQUEST_TYPE_CASCADE, } = {}) {
    const parts = [
        encodeMessage(1, metadata),
        encodeString(2, prompt ?? ''),
    ];
    for (const entry of chatMessagePrompts)
        parts.push(encodeMessage(3, entry));
    parts.push(encodeUint(7, requestType));
    if (configuration)
        parts.push(encodeMessage(8, configuration));
    for (const tool of tools)
        parts.push(encodeMessage(10, encodeChatToolDefinition(tool)));
    ...
    if (chatModelUid)
        parts.push(encodeString(21, chatModelUid));
    if (executionId)
        parts.push(encodeString(22, executionId));
    return Buffer.concat(parts);
}
