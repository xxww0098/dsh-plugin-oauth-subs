...
const FAMILIES = Object.freeze(['codex', 'grok', 'glm', 'kiro', 'antigravity', 'cursor', 'ollama', 'kimi', 'copilot', 'devin', 'cline']);
...
export function startEffortRestore({ ctx, settings, memory, prefix, effortsFor }) {
    ...
    const handle = async (next, prev) => {
        ...
        if (!snap || !action.restore || action.restore === snap.reasoningEffort)
            return;
