...
import { KIRO_STABLE_SESSION, classifyKiroHopError, kiroChatHeaders, kiroChatUrl, kiroClientErrorBody, kiroConversationId, kiroToOpenai, kiroToOpenaiChunk, KiroEventStreamParser, mapKiroUsage, mergeKiroText, openaiToKiro, resolveKiroUsage, thinkingTextFromPayload, unwrapKiroEventPayload, } from './kiro/request.js';
...
import { applyOpencodeThinking, chatToOpencodeResponses, createOpencodeResponsesChatStream, foldOpencodeReasoningContent, opencodeResponsesToChat, parseOpencodeSseBlocks, } from './opencode/request.js';
...
export function createProxy({ port, apiKey, tokens, fetchFn = fetch, maxRequestBodyBytes = MAX_REQUEST_BODY_BYTES, onAntigravityValidation, cursorRpc }) {
    ...
    const handle = async (request, response) => {
        ...
        if ((path === '/opencode/v1/models' || path === '/opencode/models') && request.method === 'GET') {
            send(response, 200, {
                object: 'list',
                data: opencodeCatalogModels().map((model) => ({ id: model.id, object: 'model', owned_by: 'opencode' })),
            });
            return;
        }
...
function sendKiroUpstreamError(response, status, text, headers) {
    ...
    send(response, classified.status, kiroClientErrorBody(status, parsed, text), classified.retryAfter ? { 'retry-after': classified.retryAfter } : {});
}
...
/**
 * OpenCode Go Free hop. Completions stay on /zen/go/v1/chat/completions.
 * Luna / Grok 4.x / Muse Spark are Go Responses: chat → /zen/go/v1/responses.
 * Bearer comes from the stored Go API key. Official `x-opencode-session`
 * is the DSH pin (never Date.now()).
 */
async function forwardOpencode(request, response, { session, fetchFn, maxRequestBodyBytes, signal, asChat }) {
    const raw = await readBody(request, maxRequestBodyBytes);
    ...
    if (!asChat && !muse) {
        send(response, 400, {
            error: {
                message: 'OpenCode Go Free Responses is Luna / Grok 4.x / Muse Spark only. Point other Go models at POST /opencode/v1/chat/completions.',
            },
        });
        return;
    }
    ...
    const headers = {
        ...opencodeUpstreamHeaders(session),
        ...opencodeCacheHeaders(cacheSessionId),
        'content-type': request.headers['content-type'] ?? 'application/json',
        ...(stream ? { accept: 'text/event-stream' } : {}),
    };
    if (!muse) {
    ...
    if (!reader) {
        ...
        if (done)
            await writeOpencodeSse(response, done, signal);
        response.write('data: [DONE]\n\n');
        response.end();
        return;
    }
    while (true) {
        ...
        for (const event of parsed.events) {
            ...
            if (chunk)
                await writeOpencodeSse(response, chunk, signal);
        }
        if (done)
            break;
    }
    ...
    if (tail)
        await writeOpencodeSse(response, tail, signal);
    response.write('data: [DONE]\n\n');
    if (!response.writableEnded && !response.destroyed)
        response.end();
}
