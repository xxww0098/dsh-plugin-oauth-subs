...
import { KIRO_STABLE_SESSION, kiroChatHeaders, kiroChatUrl, kiroClientErrorBody, kiroClientErrorStatus, kiroConversationId, kiroToOpenai, kiroToOpenaiChunk, KiroEventStreamParser, mapKiroUsage, mergeKiroText, openaiToKiro, resolveKiroUsage, unwrapKiroEventPayload, } from './kiro/request.js';
...
async function forwardAntigravity(request, response, { session, tokens, fetchFn, maxRequestBodyBytes, signal, onValidation }) {
    ...
    const headers = {
        ...antigravityChatHeaders(session, { model: payload.model }),
        ...(stream ? { accept: 'text/event-stream' } : {}),
    };
    let upstream;
    try {
