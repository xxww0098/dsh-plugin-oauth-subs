...
export function createProxy({ port, apiKey, tokens, fetchFn = fetch, maxRequestBodyBytes = MAX_REQUEST_BODY_BYTES, upstreamIdleTimeoutMs = UPSTREAM_IDLE_TIMEOUT_MS, onAntigravityValidation = undefined, cursorRpc = undefined, devinChat = undefined }) {
    let server;
...
async function forward(request, response, { url, fallbackUrl = undefined, session, tokens, headersOf, fetchFn, family, wire = undefined, maxRequestBodyBytes, upstreamIdleTimeoutMs, signal }) {
    ...
    for (let attempt = 0; attempt < STREAM_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            if (!(error instanceof RetryableUpstream))
                throw error;
 ...
 */
async function attemptUpstream(response, { url, headers, body, stream, fetchFn, family, wire, upstreamIdleTimeoutMs, signal, fallbackStatuses }) {
    let upstream;
    try {
    ...
    if (upstream.status >= 400) {
        ...
        if (upstream.status === 401)
            throw new UnauthorizedUpstream(parsed);
        if (fallbackStatuses?.has(upstream.status))
            throw new GatewayUpstream(upstream.status, parsed);
