 ...
 */
import { BUILDER_ID_START_URL, KIRO_DEFAULT_REGION, KIRO_OIDC_SCOPES, kiroSession, oidcEndpoint, } from './index.js';
...
export async function registerKiroOidcClient({ region = KIRO_DEFAULT_REGION, startUrl, fetchFn = fetch, signal } = {}) {
    ...
    if (!result.ok)
        throw new Error(`kiro oidc register failed (HTTP ${result.status}): ${result.text.slice(0, 240)}`);
    ...
    if (!clientId || !clientSecret)
        throw new Error('kiro oidc register returned no clientId/clientSecret');
        ...
        void (async () => {
            ...
            while (!controller.signal.aborted) {
                ...
                if (Date.now() >= deadline)
                    throw new Error(`login timed out after ${Math.round(expiresInSec)}s`);
                ...
                if (error === 'access_denied')
                    throw new Error('login declined on the authorization page');
