...
import { KIRO_CONTEXT_WINDOW, KIRO_DEFAULT_REGION, KIRO_MODELS, kiroUsageHeaders, kiroUsageHost, } from './index.js';
...
export function unwrapKiroEventPayload(payload, type) {
    if (!isPlainObject(payload))
        return {};
    if (type && isPlainObject(payload[type]))
        return payload[type];
...
export function normalizeToolUseId(id) {
    ...
    if (!raw)
        return undefined;
    if (raw.startsWith('tooluse_') && KIRO_TOOL_USE_ID_PATTERN.test(raw))
        return raw;
    const prefixed = raw.startsWith('tooluse_') ? raw : `tooluse_${raw.replace(/^(toolu_|call_|tool_)/, '')}`;
    if (KIRO_TOOL_USE_ID_PATTERN.test(prefixed))
        return prefixed;
...
function parseEventHeaders(buffer) {
    ...
    while (offset < buffer.length) {
        ...
        offset += 1;
        if (offset + nameLen + 1 > buffer.length)
            break;
...
function usageFromPayload(data) {
    ...
    if (nested)
        return mapKiroUsage(nested);
    if (numberField(data, 'uncachedInputTokens', 'uncached_input_tokens') !== undefined
...
export function resolveKiroUsage(collected, model) {
    if (hasRealKiroUsage(collected?.usage))
