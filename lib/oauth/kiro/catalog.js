...
import { KIRO_CONTEXT_WINDOW, KIRO_DEEPSEEK_CONTEXT, KIRO_GPT_CONTEXT, KIRO_LARGE_CONTEXT, KIRO_LIST_MODELS_PATH, KIRO_LIST_PROFILES_PATH, KIRO_MAX_TOKENS, KIRO_MODELS, KIRO_QWEN_CONTEXT, KIRO_REASONING_CLAUDE, KIRO_REASONING_CLAUDE_XHIGH, KIRO_REASONING_GPT, KIRO_TEXT_INPUT, KIRO_USAGE_REGIONS, KIRO_VISION_INPUT, kiroManagementHost, kiroStreamingProfileArn, kiroUsageHeaders, kiroUsageRegions, } from './index.js';
...
function inferKiroInput(id) {
    ...
    if (key === 'auto' || key.startsWith('claude-') || key.startsWith('gpt-'))
...
export function toKiroPickerModels(live, fallback = KIRO_MODELS) {
    ...
    for (const row of fallback ?? [])
        byId.set(row.id, { ...row, input: [...row.input] });
    for (const row of liveRows(live)) {
        const existing = byId.get(row.id);
        byId.set(row.id, kiroModelRow(row.id, row.name || existing?.name || humanizeKiroModelId(row.id), row.contextWindow || existing?.contextWindow || inferKiroWindow(row.id), row.maxTokens || existing?.maxTokens || KIRO_MAX_TOKENS, existing?.input ?? inferKiroInput(row.id), existing?.reasoningEfforts ?? inferKiroReasoning(row.id)));
    }
    const out = [];
    const seen = new Set();
    for (const row of fallback ?? []) {
