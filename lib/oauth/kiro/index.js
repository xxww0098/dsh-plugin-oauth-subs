...
export function canonicalizeKiroMethod(value, { tokenEndpoint } = {}) {
    ...
    if (lower === 'builder-id' || lower === 'builder_id' || lower === 'builderid' || lower === 'builder' || lower === 'iam')
        return 'idc';
    if (lower === 'api_key' || lower === 'apikey' || lower === 'ksk')
        return 'api_key';
    if (EXTERNAL_IDP_ALIASES.some((alias) => alias === lower))
        return 'external_idp';
    if (lower === 'oauth' || lower === 'github' || lower === 'google' || lower === 'gmail' || lower === 'gh' || lower === 'social')
        return 'social';
...
export function inferKiroAuthMethod(raw = {}) {
    ...
    if (kiroApiKey || (access && access.startsWith('ksk_')))
        return 'api_key';
    ...
    if (SOCIAL_PROVIDERS.includes(provider))
        return 'social';
    if (IDC_PROVIDERS.includes(provider))
        return 'idc';
    if (EXTERNAL_IDP_ALIASES.includes(provider))
        return 'external_idp';
...
export function allocateKiroMachineId(prior) {
    if (typeof prior === 'string')
        return kiroMachineId({ machineId: prior });
    if (prior && typeof prior === 'object')
        return kiroMachineId(prior);
...
export class KiroHttpError extends Error {
    constructor(message, status, { retryAfter } = {}) {
        ...
        if (retryAfter != null && String(retryAfter).trim())
            this.retryAfter = String(retryAfter).trim();
    }
}
...
async function readJson(response, label) {
    ...
    if (!response.ok) {
        throw new KiroHttpError(`${label} failed (HTTP ${response.status})${text ? `: ${text.slice(0, 240)}` : ''}`, response.status, { retryAfter: headerOf(response, 'retry-after') });
    }
...
export function isKiroCredential(raw) {
    ...
    const nested = raw.credentials && typeof raw.credentials === 'object' && !Array.isArray(raw.credentials)
        ? raw.credentials
        : raw;
    if (trimmed(nested.kiroApiKey ?? nested.kiro_api_key ?? raw.kiroApiKey ?? raw.kiro_api_key))
        return true;
    ...
    if (typeof start === 'string' && /awsapps\.com\/start/i.test(start) && (nested.clientId || nested.client_id || raw.clientId || raw.client_id))
        return true;
    ...
    if (refresh && refresh.length >= 100 && (trimmed(raw.email) || trimmed(nested.email)
