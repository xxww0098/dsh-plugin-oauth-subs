...
function sendKiroUpstreamError(response, status, text, headers) {
    ...
    sendJson(response, classified.status, kiroClientErrorBody(status, parsed, text), classified.retryAfter ? { 'retry-after': classified.retryAfter } : {});
}
...
export async function forwardKiro(response, { payload, cacheSessionId, stream, session, fetchFn, signal }) {
    ...
    try {
        ...
        if (!response.headersSent)
            sendKiroUpstreamError(response, 502, message, upstream.headers);
        else {
