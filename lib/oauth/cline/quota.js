 ...
 */
import { CLINE_CAP_FIELDS, CLINE_COST_SCALE, CLINE_CREDIT_SCALE, CLINE_ME_URL, CLINE_PLAN_LIMITS_URL, CLINE_PLAN_URL, CLINE_QUOTA_WINDOWS, CLINE_USAGE_BILLING, clineBalanceUrl, clineBearer, parseClineUserInfo, } from './index.js';
...
export function parseClinePlan(payload) {
    ...
    if (!planType && !Number.isFinite(periodEnd) && Object.keys(caps).length === 0)
        return undefined;
    return {
        ...(planType ? { planType } : {}),
...
export async function fetchClineQuota(session, fetchFn = fetch) {
    ...
    try {
        ...
        if (!userId)
            throw new Error('cline account id is unavailable');
