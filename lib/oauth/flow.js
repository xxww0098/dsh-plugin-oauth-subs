...
export function oauthCallbackFromUrl(url, fallbackPath) {
    const loginOption = (url.searchParams.get('login_option') ?? url.searchParams.get('loginOption') ?? '').trim().toLowerCase();
    const issuerUrl = url.searchParams.get('issuer_url') ?? url.searchParams.get('issuerUrl') ?? undefined;
    return {
        pathname: url.pathname || fallbackPath || '/',
        loginOption: loginOption || undefined,
        issuerUrl: issuerUrl || undefined,
    };
}
function oauthCallbackFromPasted(raw, fallbackPath) {
    const trimmed = String(raw ?? '').trim();
    if (/^https?:\/\//i.test(trimmed))
        return oauthCallbackFromUrl(new URL(trimmed), fallbackPath);
    if (trimmed.includes('code=')) {
        return oauthCallbackFromUrl(new URL(`http://localhost${fallbackPath || '/'}?${trimmed.replace(/^\?/, '')}`), fallbackPath);
    }
    ...
    for (const candidate of candidates) {
        ...
        for (const host of hosts) {
            ...
            try {
                ...
                if (port === 0 && typeof address === 'object')
                    port = address.port;
                servers.push(server);
            }
            catch (error) {
