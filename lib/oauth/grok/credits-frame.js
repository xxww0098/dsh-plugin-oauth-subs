...
function timestampMs(field) {
    if (!field || field.wireType !== WIRE_LEN || !field.bytes)
        return undefined;
...
export function decodeGrokCreditsFrame(buffer) {
    try {
        ...
        // 2026-09 schema: period lives in nested field 8 {type, start, end};
        // older captures keep start/end at nested fields 4/5.
        const periodField = credits.get(8) ?? top.get(8);
        const period = periodField?.wireType === WIRE_LEN ? decodeFields(periodField.bytes) : undefined;
        const resetAt = timestampMs(period?.get(3))
            ?? timestampMs(credits.get(5))
            ?? timestampMs(credits.get(4));
        const periodStart = timestampMs(period?.get(2));
        if (usedPercent === undefined && resetAt === undefined && periodStart === undefined) {
            return undefined;
        }
        // Proto3 omits zero-valued fields: a period-bearing frame without a
        // usage float means the pool is untouched (0% used), same as the web UI.
        return { usedPercent: usedPercent ?? 0, resetAt, periodStart };
    }
    catch {
        return undefined;
    }
