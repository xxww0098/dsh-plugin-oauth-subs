...
export const GROK_REASONING_47 = Object.freeze({
    low: 'low',
    medium: 'medium',
    high: 'high',
    xhigh: 'xhigh',
...
export async function grokDiscovery(fetchFn = fetch) {
    ...
    if (!response.ok)
        throw await oauthError(response, 'grok OIDC discovery');
...
export function grokTierFromValue(value) {
    ...
    if (typeof value === 'string' && value.trim()) {
        ...
        if (/^\d+$/.test(trimmed))
            return grokTierFromValue(Number(trimmed));
        return trimmed;
    }
    return undefined;
}
...
export async function exchangeGrokCode(code, verifier, redirectUri, challenge, fetchFn = fetch) {
    ...
    if (response.status === 403) {
        throw new OAuthEndpointError('grok token endpoint refused the exchange (HTTP 403): your X plan does not include the API OAuth entitlement; an X Premium or xAI subscription with API access is required', 403);
    }
    if (!response.ok)
        throw await oauthError(response, 'grok');
