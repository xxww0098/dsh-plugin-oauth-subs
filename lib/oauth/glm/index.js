...
function humanFromObject(value) {
    if (!value || typeof value !== 'object')
        return undefined;
    return pickGlmHumanAccount(value.email, value.mail, value.preferred_username, value.preferredUsername, value.username, value.userName, value.customerName, value.nickName, value.nickname, value.displayName, value.name, value.phone, value.mobile);
}
...
export async function resolveGlmIdentity(source, { fetchFn = fetch } = {}) {
    const fromHand = pickGlmHumanAccount(source?.email, source?.account, source?.accountId, accountFromJwt(source?.zcodeJwt), accountFromJwt(source?.accessToken), accountFromJwt(source?.oauthAccess));
    if (fromHand)
        return fromHand;
