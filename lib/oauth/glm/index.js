...
export function normalizeGlmPlanKind(value) {
    ...
    if (raw === 'start' || raw === 'start_plan' || raw === 'trial')
        return 'start';
    return 'coding';
}
...
export function glmPlanKindFromZcode(key, options) {
    ...
    if (/zcode-plan|start[._-]?plan/.test(hay))
        return 'start';
    return 'coding';
}
...
export function glmAnthropicUrl(region = 'zai', planKind = undefined) {
    if (normalizeGlmPlanKind(planKind) === 'start')
        return GLM_START_ANTHROPIC_URL;
    return normalizeGlmRegion(region) === 'bigmodel'
...
export function annotateGlmStartPlanError(parsed, url) {
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
        return parsed;
    if (!String(url ?? '').includes('zcode-plan'))
        return parsed;
    const code = parsed.code ?? parsed.error?.code;
    const msg = String(parsed.msg ?? parsed.message ?? parsed.error?.message ?? '');
    if (code !== 3007 && !/captcha verify failed/i.test(msg))
        return parsed;
    return {
        ...parsed,
        error: {
