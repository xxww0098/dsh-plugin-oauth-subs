...
function withMessageCacheControl(message) {
    ...
    for (let index = blocks.length - 1; index >= 0; index--) {
        ...
        if (!isPlainObject(block) || !CACHEABLE_BLOCK_TYPES.has(block.type))
            continue;
...
export function stabilizeGlmAnthropicMessageCache(messages) {
    ...
    const next = messages.map((message) => {
        if (!isPlainObject(message) || message.role === 'system')
            return message;
...
export function applyGlmCache(payload) {
    ...
    if (sessionId && (next.user == null || next.user === ''))
        next.user = sessionId;
...
export function applyGlmAnthropicCache(payload) {
    ...
    if (sessionId) {
        ...
        if (metadata.user_id == null || metadata.user_id === '')
