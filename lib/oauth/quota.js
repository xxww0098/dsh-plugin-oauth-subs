...
import { OLLAMA_ME_URL, OLLAMA_USAGE_URL, ollamaUpstreamHeaders, parseOllamaMe, } from '../apikey/ollama/index.js';
...
function ollamaWindowResetAt(window, kind, now = Date.now()) {
    ...
    if (kind === 'weekly')
        return ollamaWeeklyResetAt(now);
    return undefined;
}
    ...
    try {
        ...
        if (mcpHeaders) {
            ...
            try {
                ...
                if (mcpRow)
                    return { ...parsed, rows: finalizeGlmRows([...parsed.rows, mcpRow]) };
            }
            catch {
