...
import { ANTIGRAVITY_LOAD_CODE_ASSIST_URL, ANTIGRAVITY_QUOTA_GROUPS, antigravityFetchModelsUrls, antigravityLoadCodeAssistBody, antigravityLoadCodeAssistHeaders, antigravityPlanType, extractCloudaicompanionProject, } from './antigravity/index.js';
...
export function pickAntigravityPlanName(payload) {
    ...
    if (typeof subscription === 'string' && subscription.trim())
        return subscription.trim().toUpperCase();
    for (const path of [
        ['paidTier', 'id'],
        ['currentTier', 'id'],
        ['paid_tier', 'id'],
        ...
        if (ok && typeof cur === 'string' && cur.trim())
...
async function fetchAntigravityModelWindows(accessToken, projectId, fetchFn) {
    ...
    for (const url of antigravityFetchModelsUrls()) {
        const wait = timeoutSignal(QUOTA_TIMEOUT_MS);
        try {
            ...
            if (!response.ok) {
                lastError = new Error(`antigravity fetchAvailableModels failed (HTTP ${response.status})`);
                continue;
            }
            sawSuccess = true;
            const parsed = parseAntigravityModelQuota(await readJson(response, 'antigravity fetchAvailableModels'));
