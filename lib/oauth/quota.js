...
import { GROK_BILLING_URL, GROK_CLI_USER_URL, GROK_CLIENT_VERSION, grokTierFromValue, grokUpstreamHeaders, } from './grok/index.js';
...
export function parseGrokBilling(billing, { cliUser } = {}) {
    ...
    const subscriptionTier = formatPlanLabel(grokTierFromValue(pickPlanRaw(config.subscription_tier, config.subscriptionTier, subscription?.tier, user.subscriptionTier, user.subscription_tier)));
...
function glmWindowKind(window) {
    ...
    if (/5h|5\s*hour|five.?hour/i.test(text))
        return 'primary';
