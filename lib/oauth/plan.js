...
export function formatPlanLabel(raw, family) {
    ...
    if (family === 'glm') {
        if (GLM_PLAN_NAMES[slug])
            return GLM_PLAN_NAMES[slug];
        if (GLM_PLAN_NAMES[compact])
            return GLM_PLAN_NAMES[compact];
    }
    if (family === 'kiro') {
        if (KIRO_PLAN_NAMES[slug])
            return KIRO_PLAN_NAMES[slug];
        if (KIRO_PLAN_NAMES[compact])
            return KIRO_PLAN_NAMES[compact];
    }
    if (family === 'antigravity') {
        if (ANTIGRAVITY_PLAN_NAMES[slug])
            return ANTIGRAVITY_PLAN_NAMES[slug];
        if (ANTIGRAVITY_PLAN_NAMES[compact])
            return ANTIGRAVITY_PLAN_NAMES[compact];
    }
    if (family === 'cursor') {
        if (CURSOR_PLAN_NAMES[slug])
            return CURSOR_PLAN_NAMES[slug];
        if (CURSOR_PLAN_NAMES[compact])
            return CURSOR_PLAN_NAMES[compact];
    }
    if (family === 'ollama') {
        if (OLLAMA_PLAN_NAMES[slug])
            return OLLAMA_PLAN_NAMES[slug];
        if (OLLAMA_PLAN_NAMES[compact])
            return OLLAMA_PLAN_NAMES[compact];
    }
    if (family === 'copilot') {
        if (COPILOT_PLAN_NAMES[slug])
            return COPILOT_PLAN_NAMES[slug];
        if (COPILOT_PLAN_NAMES[compact])
            return COPILOT_PLAN_NAMES[compact];
    }
    if (family === 'cline') {
        if (CLINE_PLAN_NAMES[slug])
            return CLINE_PLAN_NAMES[slug];
        if (CLINE_PLAN_NAMES[compact])
            return CLINE_PLAN_NAMES[compact];
    }
    if (family === 'devin') {
        if (DEVIN_PLAN_NAMES[slug])
            return DEVIN_PLAN_NAMES[slug];
        if (DEVIN_PLAN_NAMES[compact])
            return DEVIN_PLAN_NAMES[compact];
        // `teams_tier` may arrive as a numeric string (16 = Devin Pro).
        const tier = Number(trimmed);
        if (/^\d+$/.test(trimmed) && DEVIN_TIER_NAMES[tier])
            return DEVIN_TIER_NAMES[tier];
    }
    if (family !== 'glm' && family !== 'grok' && family !== 'kiro' && family !== 'antigravity' && family !== 'cursor' && family !== 'ollama' && family !== 'kimi' && family !== 'copilot' && family !== 'devin' && family !== 'cline') {
        ...
        if (CODEX_PLAN_NAMES[compact])
            return CODEX_PLAN_NAMES[compact];
    }
    if (GLM_PLAN_NAMES[slug] && family !== 'codex')
        return GLM_PLAN_NAMES[slug];
