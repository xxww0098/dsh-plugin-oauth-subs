...
export function antigravityCloudCodeFallbacks(url) {
    ...
    if (!href.startsWith(ANTIGRAVITY_DAILY_API_URL))
        return [href];
...
export async function fetchAntigravityCloudCode(url, init, fetchFn = fetch) {
    ...
    for (let i = 0; i < urls.length; i++) {
        ...
        try {
            ...
            if (response.ok || !retryHubOnProd(response) || last)
                return response;
        }
        catch (error) {
            lastError = error;
...
export function normalizeAntigravityVersion(value) {
    ...
    if (!new RegExp(`^${VERSION_TOKEN.source}$`).test(raw))
        return undefined;
 ...
 */
export function detectAntigravityVersion({ platform = process.platform, env = process.env, readFile = (path) => readFileSync(path, 'utf8'), execFile = (file, args, opts) => execFileSync(file, args, opts), } = {}) {
    if (platform === 'darwin') {
        try {
...
export function antigravityLoadCodeAssistBody(projectId) {
    ...
    if (!pid)
        return { metadata: antigravityLoadCodeAssistMetadata() };
    return {
...
export function defaultAntigravityTierId(loadResp) {
    ...
    for (const tier of tiers) {
        if (tier?.isDefault && trimmed(tier.id))
...
function googleAiClaim(value) {
    ...
    if (!claim || isCodeAssistOnlyPlan(claim))
        return undefined;
    return claim;
}
/**
...
function walkValidationUrl(value, depth = 0) {
    ...
    if (typeof value === 'string') {
        ...
        if (/^https:\/\/accounts\.google\.com\//i.test(href) && /[?&]plt=/.test(href))
            return href;
        if (/^https:\/\/accounts\.google\.com\//i.test(href) && /signin\/continue/i.test(href))
            return href;
        return undefined;
    }
...
export function antigravitySession({ accessToken, refreshToken, expiresAt, expiresIn, account, projectId, planType, needsValidation, validationUrl, } = {}) {
    if (!trimmed(accessToken))
        throw new Error('antigravity session needs an access token');
    if (!trimmed(refreshToken))
        throw new Error('antigravity session needs a refresh token');
    if (!trimmed(projectId))
        throw new Error('antigravity session needs a project_id');
...
export async function exchangeAntigravityTokens(body, fetchFn = fetch) {
    ...
    if (!response.ok)
        throw await oauthError(response, 'antigravity token');
...
export async function fetchAntigravityUserInfo(accessToken, { fetchFn = fetch } = {}) {
    ...
    if (!email)
        throw new Error('antigravity userinfo returned no email');
    return email;
}
export async function onboardAntigravityUser(accessToken, tierId, { fetchFn = fetch, sleep = delay } = {}) {
    ...
    for (let attempt = 1; attempt <= ANTIGRAVITY_ONBOARD_ATTEMPTS; attempt++) {
        ...
        if (attempt < ANTIGRAVITY_ONBOARD_ATTEMPTS)
            await sleep(ANTIGRAVITY_ONBOARD_PAUSE_MS);
    }
...
export async function completeAntigravityLogin(tokens, { fetchFn = fetch, sleep, account } = {}) {
    ...
    if (!accessToken)
        throw new Error('antigravity token exchange returned no access token');
    if (!refreshToken)
...
export async function refreshAntigravity(session, fetchFn = fetch) {
    ...
    if (!accessToken)
        throw new Error('antigravity refresh returned no access token');
...
export async function probeAntigravityValidation(session, { fetchFn = fetch } = {}) {
    const projectId = trimmed(session?.projectId);
    if (!projectId || !trimmed(session?.accessToken))
        return undefined;
...
export function isAntigravityPermanentRefreshError(error) {
    if (parseAntigravityValidation(error) || error?.code === ANTIGRAVITY_VERIFY_CODE)
        return false;
