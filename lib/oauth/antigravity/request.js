...
function rememberThoughtSignature(sessionId, { id, name, args, signature } = {}) {
    ...
    if (ck)
        bucket.set(ck, signature);
    if (trimmed(id))
        bucket.set(`id:${id}`, signature);
...
export function antigravityMaxOutputTokens(model) {
    const id = String(model ?? '');
    if (RUNTIME_MAX_OUTPUT_TOKENS[id] !== undefined)
        return RUNTIME_MAX_OUTPUT_TOKENS[id];
    if (id.startsWith('claude-'))
        return 64_000;
    if (id.startsWith('gpt-oss-'))
        return 32_768;
    if (id.startsWith('gemini-3.1-pro') || id === 'gemini-pro-agent')
        return 65_535;
    if (id.startsWith('gemini-'))
        return 65_536;
    return 8192;
...
function toolDeclarations(tools, model) {
    ...
    const declarations = tools.flatMap((tool) => {
        ...
        if (schema) {
            if (legacy)
                decl.parameters = normalizeCustomToolSchema(schema);
            else
...
function geminiToolChoiceMode(toolChoice) {
    ...
    if (value === 'none')
        return 'NONE';
    if (value === 'required' || value === 'any' || value === 'function')
        return 'ANY';
    return 'AUTO';
}
...
export function antigravityThinkingConfig(model, effort) {
    if (isClaudeModel(model) || isGptOssModel(model))
        return undefined;
...
export function openaiToAntigravity(payload, { projectId, sessionId } = {}) {
    ...
    if (pinned.extra)
        appendTurn(contents, 'user', [{ text: pinned.extra }]);
    const request = {
        contents: mergeAdjacentContents(contents),
        sessionId: pinnedSession,
    };
    if (pinned.parts.length)
        request.systemInstruction = { role: 'user', parts: pinned.parts };
...
function openaiChunk({ id, model, delta, finish_reason, usage = undefined }) {
    const chunk = {
        id,
    ...
    function applyEvent(body) {
        ...
        if (collected.toolCalls.length) {
            ...
            for (let index = 0; index < collected.toolCalls.length; index++) {
                ...
                if (first && call.thoughtSignature)
                    attachThoughtSignatureFields(next, call.thoughtSignature);
                calls.push(next);
            }
            if (calls.length)
