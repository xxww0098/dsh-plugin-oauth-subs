...
function opencodeHarnessModels(opencodeModels) {
    if (Array.isArray(opencodeModels) && opencodeModels.length > 0)
...
export function buildProviders({ prefix, origin, loggedIn, cursorModels, ollamaModels, kiroModels, kimiModels, opencodeModels }) {
    ...
    if (loggedIn.opencode) {
        providers[`${prefix}-opencode`] = {
            ...
            // Omit reasoningEfforts (do not write false) — a boolean on every
            // Completions row can abort the atomic llm-pi-ai mutate so the
            // family never lands in settings.yaml. No Completions compat.
            baseURL: `${origin}/opencode`,
            models: opencodeHarnessModels(opencodeModels).map((model) => {
                const row = toHarnessModel(model);
...
export function familyOfProvider(provider) {
    ...
    if (String(provider).endsWith('-kimi'))
        return 'kimi';
    if (String(provider).endsWith('-opencode'))
        return 'opencode';
...
export class ModelSwitch {
    ...
    async setFamily(family, on, catalog) {
        if (!FAMILY_IDS.includes(family))
            throw new Error('family must be codex, grok, glm, kiro, antigravity, cursor, ollama, kimi, or opencode');
