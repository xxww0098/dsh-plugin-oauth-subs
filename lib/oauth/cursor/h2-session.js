 ...
 * Do not add Bun.
 */
import { CURSOR_AGENT_URL, CURSOR_API2_URL, CURSOR_AVAILABLE_MODELS_PATH, CURSOR_MODELS_PATH, CURSOR_RUN_PATH, cursorAgentUrl, cursorChatHeaders, cursorUpstreamProxy, } from './index.js';
...
function describeCursorRunError(message) {
    ...
    if (/not supported in your region/i.test(text) && !cursorUpstreamProxy()) {
        return (`${text} Route the Cursor hop through a supported-region egress: ` +
            'plugin config "cursorProxy" or env CURSOR_PROXY / PI_CURSOR_PROXY (http:// or socks5://).');
    }
    return text;
}
export async function cursorUnaryRpc({ session, url = CURSOR_API2_URL, path, body = Buffer.alloc(0), connectFn = cursorH2Connect, signal, timeoutMs = 8_000, }) {
 ...
 */
export async function runCursorAgent(session, built, { signal, connectFn = cursorH2Connect, url = cursorAgentUrl() || CURSOR_AGENT_URL, onEvent, } = {}) {
    signal?.throwIfAborted();
