...
export function displayCursorAccount(session) {
    return pickCursorHumanAccount(session?.account, cursorAccountFromToken(session?.accessToken), session?.cachedEmail);
}
...
export function cursorSession({ accessToken, refreshToken, expiresAt, account, planType, source = 'pkce', } = {}) {
