...
function dialSocket({ host, port, secure, servername = undefined, timeoutMs }) {
...
async function socks5Tunnel(proxy, target, timeoutMs) {
    ...
    try {
        ...
        if (host.length > 255)
            throw new Error('SOCKS5 target host exceeds 255 bytes');
        const port = targetPort(target);
        ...
        if (addrLen < 0)
            throw new Error(`SOCKS5 reply uses unknown address type ${atyp}`);
...
export async function dialCursorProxy(proxy, target, { timeoutMs = DIAL_TIMEOUT_MS } = {}) {
    ...
    if (protocol === 'http' || protocol === 'https')
        return httpConnectTunnel(url, target, timeoutMs);
