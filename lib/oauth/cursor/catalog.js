...
import { cursorCatalogCache, setCursorParamStyles } from './registry.js';
        ...
        for (const variant of model.variants ?? []) {
            ...
            for (const parameter of variant.parameters ?? []) {
                if (EFFORT_PARAM_IDS.has(parameter.id)) {
                    ...
                    if (key)
                        style.efforts[key] = parameter.value;
                }
...
function cursorModelRow(id, name, contextWindow, maxTokens, input = CURSOR_VISION, efforts = CURSOR_REASONING) {
    return {
        id,
        name,
        contextWindow,
...
export function toCursorPickerModels(usable, parameterized = []) {
    ...
    for (const [id, group] of groups) {
        ...
        const window = clampCursorContextWindow(id, name, contexts.length ? Math.max(...contexts) : group.windows.length ? Math.max(...group.windows) : inferredWindow);
...
export async function refreshCursorCatalog(session, options = {}) {
    ...
    if (cursorCatalogCache.tokenHash === tokenHash && cursorCatalogCache.models?.length)
        return cursorCatalogCache.models;
    return mergeCursorStaticFloor([]);
}
