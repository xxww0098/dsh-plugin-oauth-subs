...
import { cursorConversationId, cursorStableId, peelCursorFastSuffix, pinCursorSystemPrefix, } from './cache.js';
...
function vendorEffort(style, value) {
    if (typeof value !== 'string' || !value.trim() || !style?.effortParam)
        return undefined;
    const key = value.trim();
    const efforts = style.efforts ?? {};
    if (Object.values(efforts).includes(key))
        return key;
    if (efforts[key])
        return efforts[key];
    const legacy = CURSOR_REASONING[key];
    if (legacy && Object.values(efforts).includes(legacy))
        return legacy;
    return undefined;
}
/**
...
export function cursorModelParameters(payload = {}) {
    ...
    if (effort)
        parameters.push({ id: style.effortParam, value: effort });
    if (style.fast === true) {
...
export function openaiToCursor(payload = {}, { conversationId } = {}) {
    ...
    const continuationText = parsed.continuation
        ? parsed.toolResults
            ...
            if (result.isError)
                return `Tool error: ${body || 'unknown error'}`;
    ...
    const userMessageId = cursorStableId(resolvedId, 'user', currentText, JSON.stringify(parsed.inFlight?.steps ?? []));
...
export function createCursorOpenaiStream({ model, id, conversationId }) {
    ...
    return {
        ...
        push(event) {
            ...
            if (event?.usage) {
                if (Number.isFinite(event.usage.promptTokens))
                    collected.usage.promptTokens = event.usage.promptTokens;
                if (Number.isFinite(event.usage.completionTokens))
                    collected.usage.completionTokens = event.usage.completionTokens;
                if (Number.isFinite(event.usage.cachedTokens))
                    collected.usage.cachedTokens = event.usage.cachedTokens;
            }
