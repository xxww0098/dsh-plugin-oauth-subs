...
export function mapOpencodeUsage(usage) {
    ...
    const details = isPlain(usage.input_tokens_details)
        ...
        : isPlain(usage.prompt_tokens_details)
            ...
            : undefined;
    const cached = firstNumber(details?.cached_tokens, usage.cache_read_input_tokens, usage.cache_read_tokens, usage.cached_tokens);
    const next = { prompt_tokens: prompt, completion_tokens: completion, total_tokens: total };
    if (cached !== undefined)
        next.prompt_tokens_details = { cached_tokens: cached };
    return next;
}
...
export function foldOpencodeReasoningContent(payload) {
    ...
    if (isPlain(payload.usage))
        next.usage = mapOpencodeUsage(payload.usage);
    return next;
}
    ...
    return {
        push(event) {
            ...
            if (event.type === 'response.completed' || event.type === 'response.failed' || event.type === 'error') {
                ...
                return chatChunk(chatId, model, {}, finishReasonOf(response, collected.toolCalls), isPlain(response?.usage) ? mapOpencodeUsage(response.usage) : undefined);
            }
            return undefined;
        },
        finish() {
