...
export function parseOpencodeApiKey(value) {
    ...
    if (!key || key.length < 8)
        throw new Error('opencode API key is empty');
    if (key === OPENCODE_ANON_TOKEN)
        throw new Error('opencode anonymous sentinel is not a Go API key');
    return key;
}
...
export function opencodeSession({ accessToken, account, source = 'paste', planType = 'go', } = {}) {
...
export async function refreshOpencode(session) {
    if (isOpencodeAnonSession(session))
        throw new Error('opencode anonymous sentinel is not a Go API key');
...
export function opencodeUpstreamHeaders(session) {
    ...
    if (key && key !== OPENCODE_ANON_TOKEN)
