 ...
 */
import { OPENCODE_DEFAULT_MAX_TOKENS, OPENCODE_INPUT, OPENCODE_KEYED_FREE, OPENCODE_MODELS, OPENCODE_MODELS_DEV_URL, OPENCODE_MODELS_URL, OPENCODE_REFERER, OPENCODE_TITLE, OPENCODE_USER_AGENT, OPENCODE_VISION_INPUT, isOpencodeFreeSlug, opencodePrettyName, opencodeUpstreamHeaders, } from './index.js';
...
export function toOpencodePickerModels(payload) {
    ...
    for (const row of rows) {
        ...
        if (!id || !isOpencodeFreeSlug(id) || OPENCODE_KEYED_FREE.has(id.toLowerCase()) || seen.has(id))
            continue;
        seen.add(id);
        models.push({
            id,
