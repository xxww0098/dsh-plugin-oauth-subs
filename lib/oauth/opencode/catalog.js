 ...
 */
import { OPENCODE_DEFAULT_MAX_TOKENS, OPENCODE_INPUT, OPENCODE_MODELS, OPENCODE_MODELS_DEV_URL, OPENCODE_MODELS_URL, OPENCODE_REFERER, OPENCODE_TITLE, OPENCODE_USER_AGENT, OPENCODE_VISION_INPUT, isOpencodeGoSlug, isOpencodeZenFreeSlug, opencodePrettyName, opencodeUpstreamHeaders, } from './index.js';
...
export function opencodeReasoningEffortsOf(dev) {
    ...
    if (effort) {
        ...
        for (const value of effort.values) {
            ...
            if (DSH_THINKING_LEVELS.includes(wire))
                map[wire] = wire;
        }
...
export function modelsDevOpencodeGoMap(payload) {
    ...
    for (const [id, row] of Object.entries(models)) {
        if (row && typeof row === 'object' && !isOpencodeZenFreeSlug(id))
            map.set(id, row);
    }
    return map;
}
export function applyOpencodeModelsDev(model, dev) {
    ...
    if (floor.reasoningEfforts)
        next.reasoningEfforts = { ...floor.reasoningEfforts };
    else
        delete next.reasoningEfforts;
    return next;
}
...
export function toOpencodePickerModels(payload) {
    ...
    for (const row of rows) {
        ...
        if (!id || !isOpencodeGoSlug(id) || isOpencodeZenFreeSlug(id) || seen.has(id))
            continue;
        seen.add(id);
        models.push({
            id,
...
export async function refreshOpencodeCatalog({ fetchFn = fetch, signal, force = false } = {}) {
    if (!force && cached.models?.length && Date.now() < cached.expiresAt)
