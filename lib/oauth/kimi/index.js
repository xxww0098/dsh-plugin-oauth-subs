...
export function parseKimiApiKey(value) {
    ...
    if (!key || key.length < 8)
        throw new Error('kimi API key is empty');
    return key;
}
...
export function computeKimiDeviceModel({ platform, release, arch, macVersion } = {}) {
    ...
    if (osName === 'win32') {
        ...
        if (label === '10' && parts.length >= 3) {
            ...
            if (!Number.isNaN(build) && build >= 22000)
                label = '11';
        }
        if (label && cpu)
            return `Windows ${label} ${cpu}`;
        if (label)
...
export function kimiSession({ accessToken, refreshToken, expiresAt, account, planType, source = 'oauth', } = {}) {
    ...
    if (!refresh)
        throw new Error('kimi token endpoint returned no refresh token');
...
export async function refreshKimi(session, fetchFn = fetch) {
    if (isKimiKeySource(session?.source) || session?.refreshToken === session?.accessToken) {
        if (!session?.accessToken)
            throw new Error('kimi session needs an API key');
        return session;
    }
