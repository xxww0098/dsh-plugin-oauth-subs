...
export class TokenManager {
    constructor({ provider, authPath, displayName, preemptMs, refresh, isPermanent, onRemoved, refreshWaitMs = REFRESH_WAIT_MS }) {
    ...
    async #resolve(source, { force }) {
        ...
        if (!force && left > 0) {
            ...
            if (failed && Date.now() - failed.at < REFRESH_FAILURE_BACKOFF_MS)
                return this.#serve(source);
