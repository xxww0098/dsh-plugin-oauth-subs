...
import { hydrateKiroSsoToken, kiroSsoClientIdHash, sessionsFromKiroAuth, } from './kiro/import.js';
...
function glmZcodeProviderUsable(value) {
    if (!value || typeof value !== 'object')
        return false;
    if (value.enabled === false || value.available === false)
        return false;
    if (value.options?.enabled === false || value.options?.available === false)
        return false;
    if (value.entitled === false || value.options?.entitled === false)
        return false;
    if (value.coding_plan_not_entitled === true)
        return false;
    const status = [value.status, value.error, value.reason, value.code, value.options?.status, value.options?.error]
        .filter((part) => part != null && part !== '')
        .join(' ');
    if (/not[_-]?entitled|coding_plan_not_entitled/i.test(status))
        return false;
    return true;
}
...
export function glmKeyCandidateFromZcodeConfig(raw) {
    ...
    for (const [key, value] of Object.entries(providers)) {
        if (!/zai|glm|coding.?plan|start.?plan|bigmodel|zcode/i.test(key))
            continue;
        ...
        if (looksLikeJwt(trimmed) && /start[._-]?plan|zcode-plan/.test(`${key} ${options?.baseURL ?? options?.baseUrl ?? options?.base_url ?? ''}`.toLowerCase()))
            continue;
        found.push({
            apiKey: trimmed,
...
export function decodeZcodeCredentialValue(value, { env = process.env } = {}) {
    if (typeof value !== 'string' || !value.startsWith(ZCODE_ENCRYPTED_PREFIX))
        return value;
...
export function glmKeyFromZcodeCredentials(raw, { env = process.env } = {}) {
    ...
    const provisioned = [];
    for (const [key, value] of Object.entries(raw)) {
        if (!/^account-provider:.+:api-key$/.test(key))
            continue;
...
function tokensFromAntigravityRaw(raw) {
    ...
    const access = pickString(nested.access_token, nested.accessToken, raw.access_token, raw.accessToken);
    const refresh = pickString(nested.refresh_token, nested.refreshToken, raw.refresh_token, raw.refreshToken);
    if (!access || !refresh)
        return undefined;
...
async function readAntigravityJsonFiles(dir) {
    try {
        ...
        for (const name of names) {
            if (!/^antigravity(-.+)?\.json$/i.test(name))
                continue;
            const raw = await readJson(join(dir, name));
            if (raw !== undefined)
                out.push({ path: join(dir, name), raw });
        }
        return out;
    }
    catch (error) {
...
function isClientRegistration(raw) {
    ...
    if (typeof clientId !== 'string' || typeof clientSecret !== 'string')
        return false;
    if (raw.refreshToken || raw.refresh_token || raw.accessToken || raw.access_token)
        return false;
    return true;
}
async function importKiroFromCache(dir, tried, seenIds, seenPaths) {
    ...
    for (const name of names) {
        ...
        const raw = await readJson(path);
        if (raw !== undefined)
            files.set(name, { path, raw });
    }
    const registrations = [];
    for (const { raw } of files.values()) {
        if (isClientRegistration(raw))
    ...
    for (const name of ordered) {
        ...
        tried.push(row.path);
        if (isClientRegistration(row.raw))
            continue;
        const sessions = takeKiroSessions(row.raw, name === 'kiro-auth-token.json' ? registration : undefined);
