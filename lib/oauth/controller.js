...
import { isOpencodeAnonSession, isOpencodePermanentRefreshError, opencodeSession, refreshOpencode, } from './opencode/index.js';
...
export class AuthController {
    ...
    async #forgetAnonymousOpencode() {
        ...
        for (const row of rows) {
            if (!isOpencodeAnonSession(row.session))
                continue;
    ...
    async #maybeAutoImportOpencode() {
        if (!this.opencodeAutoImport || this.opencodeAutoImportTried)
            return;
    ...
    async useKey(provider, key, extra) {
        ...
        if (provider !== 'glm')
            throw new Error('only GLM, Kiro, Ollama Cloud, Kimi, OpenCode Go Free, and Copilot accept a pasted key');
