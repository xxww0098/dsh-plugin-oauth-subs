...
export class AuthController {
    ...
    async snapshot() {
        ...
        const [codexAccounts, grokAccounts, glmAccounts, kiroAccounts, antigravityAccounts, cursorAccounts, ollamaAccounts] = await Promise.all([
    ...
    async refreshQuota(provider, accountId) {
        if (provider === 'codex' || provider === 'grok' || provider === 'glm' || provider === 'kiro' || provider === 'antigravity' || provider === 'cursor' || provider === 'ollama') {
        ...
        return { codex, grok, glm, kiro, antigravity, cursor, ollama };
    }
    ...
    async useKey(provider, key, extra) {
        ...
        if (provider !== 'glm')
            throw new Error('only GLM, Kiro, and Ollama accept a pasted key');
    ...
    async setModels(payload = {}) {
        ...
        else if (payload.family === 'codex' || payload.family === 'grok' || payload.family === 'glm' || payload.family === 'kiro' || payload.family === 'antigravity' || payload.family === 'cursor' || payload.family === 'ollama') {
