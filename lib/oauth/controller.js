...
import { GLM_MODELS, glmSession, isGlmPermanentRefreshError, normalizeGlmRegion, pickGlmHumanAccount, refreshGlm, resolveGlmIdentity, } from './glm/index.js';
import { BUILDER_ID_START_URL, allocateKiroMachineId, canonicalizeKiroMethod, exchangeKiroSocialCode, isKiroPermanentRefreshError, kiroSession, kiroSocialFlow, refreshKiro, refreshKiroExternalIdp, refreshKiroSocial, validateKiroApiKey, validateKiroIdpEndpoint, validateKiroRefreshToken, } from './kiro/index.js';
import { isKiroBatchImport, parseKiroImportText } from './kiro/import.js';
import { antigravityFlow, ANTIGRAVITY_PREEMPT_MS, applyAntigravityValidation, exchangeAntigravityCode, isAntigravityPermanentRefreshError, probeAntigravityValidation, refreshAntigravity, } from './antigravity/index.js';
...
import { cursorAccountFromToken, isCursorPermanentRefreshError, pickCursorHumanAccount, refreshCursor, } from './cursor/index.js';
...
import { completeKimiDevice as sessionFromKimiDevice, configureKimiIdentity, isKimiOpaqueAccount, isKimiPermanentRefreshError, kimiDeviceSpec, kimiSession, refreshKimi, resolveKimiIdentity, } from './kimi/index.js';
...
import { completeCopilotDevice as sessionFromCopilotDevice, isCopilotOpaqueAccount, isCopilotPermanentRefreshError, isCopilotSessionToken, copilotDeviceSpec, mintCopilotSessionFromGithub, refreshCopilot, resolveCopilotIdentity, } from './copilot/index.js';
...
import { devinFlow, devinSession, exchangeDevinCode, isDevinOpaqueAccount, isDevinPermanentRefreshError, isDevinSessionToken, pickDevinHumanAccount, refreshDevin, } from './devin/index.js';
...
import { clineDeviceSpec, isClineOpaqueAccount, isClinePermanentRefreshError, refreshCline, registerClineTokens, resolveClineIdentity, } from './cline/index.js';
...
import { APIKEY_FAMILY_IDS, buildProviders, catalogProviders, describeCatalog, describeProviders, ensureOpencodeGoRoute, filterProviders, MODEL_FAMILY_IDS, ModelSwitch, OPENCODE_GO_API_KEY_ENV, syncHarnessModels, } from './models.js';
import { TokenManager } from './tokens.js';
import { QuotaStore } from './quota.js';
import { fetchLatest, installRelease, localUpdateInfo, DEFAULT_PROFILE, } from '../utils/update.js';
import { AUTO_UPDATE_INTERVAL_MS, readUpdatePrefs, readUpdateState, updatePrefsPath, updateStatePath, writeUpdatePrefs, writeUpdateState, } from '../utils/update-prefs.js';
...
export class AuthController {
    ...
    async #discoverCursor(session) {
        if (!session || typeof this.cursorDiscover !== 'function')
    ...
    async #discoverOllama(session) {
        if (!session || typeof this.ollamaDiscover !== 'function')
    ...
    async #discoverKiro(session) {
        if (!session || typeof this.kiroDiscover !== 'function')
    ...
    async #discoverKimi(session) {
        if (!session || typeof this.kimiDiscover !== 'function')
    ...
    async #discoverCopilot(session) {
        if (!session || typeof this.copilotDiscover !== 'function')
    ...
    async #discoverDevin(session) {
        if (!session || typeof this.devinDiscover !== 'function')
    ...
    async #discoverCline(session) {
        if (!session || typeof this.clineDiscover !== 'function')
    ...
    async snapshot() {
        ...
        if (loggedIn.cursor)
            await this.#ensureAccountQuota('cursor');
        else
            this.quota.clear('cursor');
        if (loggedIn.ollama)
            await this.#ensureAccountQuota('ollama');
        else
            this.quota.clear('ollama');
        if (loggedIn.kimi)
            await this.#ensureAccountQuota('kimi');
        else
            this.quota.clear('kimi');
        if (loggedIn.copilot)
            await this.#ensureAccountQuota('copilot');
        else
            this.quota.clear('copilot');
        if (loggedIn.devin)
            await this.#ensureAccountQuota('devin');
        else
            this.quota.clear('devin');
        if (loggedIn.cline)
            await this.#ensureAccountQuota('cline');
        else
            this.quota.clear('cline');
    ...
    async #maybeAdoptOpencodeGoKey() {
        if (this.opencodeGoAdopted || !this.opencodeGo)
            return;
        this.opencodeGoAdopted = true;
        const id = this.opencodeGo.keylessId();
        if (!id || typeof this.credentials?.resolve !== 'function')
            return;
        try {
            const resolved = await this.credentials.resolve(OPENCODE_GO_API_KEY_ENV);
            if (resolved?.value)
                await this.opencodeGo.adoptKey(id, resolved.value);
        }
        catch {
    ...
    async #mirrorOpencodeGoKey(id) {
        ...
        if (key) {
            if (typeof this.credentials?.set === 'function')
                await this.credentials.set(OPENCODE_GO_API_KEY_ENV, key);
            return;
        }
        if (this.opencodeGo?.anyKey())
            return;
        if (typeof this.credentials?.unset === 'function')
            await this.credentials.unset(OPENCODE_GO_API_KEY_ENV);
    }
    ...
    async saveOpencodeGo(payload = {}) {
        if (!this.opencodeGo)
            throw new Error('OpenCode Go store is unavailable');
    ...
    async refreshQuota(provider, accountId) {
        if (provider === 'opencode-go')
            return this.refreshOpencodeGoQuota(accountId);
    ...
    async setAutoUpdate(payload = {}) {
        ...
        if (this.autoUpdate)
            void this.runAutoUpdate().catch(() => undefined);
    ...
    async runAutoUpdate() {
        ...
        if (!this.autoUpdate)
            return { skipped: true };
        const result = await this.checkUpdate({ apply: true }).catch((error) => ({
            status: 'error',
            error: error instanceof Error ? error.message : String(error),
        }));
        this.updateState = {
    ...
    async #ensureAccountQuota(provider) {
        ...
        await Promise.all(rows.map(async (row) => {
            ...
            if (provider === 'antigravity')
                await this.#rememberAntigravityPlan(row, quota);
            if (provider === 'cursor')
                await this.#rememberCursorPlan(row, quota);
            if (provider === 'ollama')
                await this.#rememberOllamaIdentity(row, quota);
            if (provider === 'kimi')
                await this.#rememberKimiIdentity(row, quota);
            if (provider === 'devin')
                await this.#rememberDevinIdentity(row, quota);
            if (provider === 'cline')
                await this.#rememberClineIdentity(row, quota);
        }));
        return rows;
    }
    ...
    async #rememberCursorPlan(row, quota) {
        ...
        if (!email && !planType && cachedEmail === row.session.cachedEmail)
            return;
        if ((!email || row.session.account === email)
    ...
    async #resolveCursorIdentities() {
        ...
        await Promise.all(rows.map(async (row) => {
            if (pickCursorHumanAccount(row.session?.account))
                return;
            const account = pickCursorHumanAccount(cursorAccountFromToken(row.session?.accessToken), this.#cachedEmailFor(row.session, vscdb));
            if (!account)
                return;
    ...
    #cachedEmailFor(session, vscdb) {
        ...
        if (session.source === 'ide_vscdb' || sameAccess || sameRefresh)
            return email;
        return undefined;
    }
    ...
    async #maybeAutoImportCursor() {
        if (!this.cursorAutoImport || this.cursorAutoImportTried)
            return;
    ...
    async #maybeAutoImportOllama() {
        if (!this.ollamaAutoImport || this.ollamaAutoImportTried)
            return;
    ...
    async #maybeAutoImportKimi() {
        if (!this.kimiAutoImport || this.kimiAutoImportTried)
            return;
    ...
    async #maybeAutoImportCopilot() {
        if (!this.copilotAutoImport || this.copilotAutoImportTried)
            return;
    ...
    async #maybeAutoImportCline() {
        if (!this.clineAutoImport || this.clineAutoImportTried)
            return;
    ...
    async #maybeAutoImportDevin() {
        if (!this.devinAutoImport || this.devinAutoImportTried)
            return;
    ...
    async completePkce(provider, attempt, claim) {
        try {
            ...
            if (provider === 'antigravity')
                void this.#probeAntigravity(saved);
        }
        catch (error) {
            if (this.claims.get(provider) !== claim)
                return;
    ...
    async useKey(provider, key, extra) {
        ...
        if (provider !== 'glm')
            throw new Error('only GLM, Kiro, Ollama Cloud, Kimi, Copilot, and Devin accept a pasted key');
    ...
    async #saveKiroImports(sessions, { refreshMissingAccess = false } = {}) {
        ...
        for (const draft of sessions) {
            ...
            try {
                if (needsRefresh)
                    session = await refreshKiro(session, { fetchFn: this.fetchFn });
        ...
        this.lastError.delete('kiro');
        if (saved[0])
            await this.#discoverKiro(saved[0]);
        this.onAuthChanged?.('kiro');
        void this.quota.refresh('kiro');
        return {
    ...
    async logout(provider, id) {
        if (provider === 'opencode-go')
            return this.logoutOpencodeGo(id);
        this.claim(provider);
        this.flows.pending(provider)?.cancel();
        this.devices.pending(provider)?.cancel();
        this.glmFlows.pending(provider)?.cancel();
    ...
    async importFrom(provider) {
        ...
        this.lastError.delete(provider);
        if (provider === 'cursor')
            await this.#discoverCursor(sessions[0]);
        if (provider === 'ollama')
            await this.#discoverOllama(sessions[0]);
        if (provider === 'kiro')
            await this.#discoverKiro(sessions[0]);
        if (provider === 'kimi')
            await this.#discoverKimi(sessions[0]);
        if (provider === 'copilot')
            await this.#discoverCopilot(sessions[0]);
        if (provider === 'devin')
            await this.#discoverDevin(sessions[0]);
        if (provider === 'cline')
            await this.#discoverCline(sessions[0]);
        this.onAuthChanged?.(provider);
        void this.quota.refresh(provider);
        return {
