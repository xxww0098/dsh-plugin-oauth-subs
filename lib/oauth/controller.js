...
import { buildProviders, catalogProviders, describeCatalog, describeProviders, ensureOpencodeGoRoute, filterProviders, ModelSwitch, OPENCODE_GO_API_KEY_ENV, syncHarnessModels, } from './models.js';
...
export class AuthController {
    ...
    async opencodeGoSnapshot(options) {
        if (!this.opencodeGo)
            return { id: 'opencode-go', cookieSet: false, workspaceId: '', configured: false, apiKeySet: false, quota: { status: 'idle' } };
    ...
    async saveOpencodeGo(payload = {}) {
        ...
        if (raw) {
            if (typeof this.credentials?.set !== 'function')
                throw new Error('credentials store is unavailable');
    ...
    async refreshQuota(provider, accountId) {
        if (provider === 'opencode-go')
            return this.refreshOpencodeGoQuota();
