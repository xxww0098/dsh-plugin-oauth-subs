...
import { isOpencodePermanentRefreshError, opencodeSession, refreshOpencode, } from './opencode/index.js';
...
export class AuthController {
    ...
    async #discoverOpencode() {
        if (typeof this.opencodeDiscover !== 'function')
    ...
    async snapshot() {
        ...
        if (loggedIn.opencode)
            await this.#ensureAccountQuota('opencode');
        else
            this.quota.clear('opencode');
    ...
    async #maybeAutoEnableOpencode({ notify = true } = {}) {
        if (!this.opencodeAutoEnable || this.opencodeAutoEnableTried)
            return;
    ...
    async useKey(provider, key, extra) {
        ...
        if (provider === 'opencode')
            throw new Error('opencode free is anonymous — no API key');
        if (provider !== 'glm')
    ...
    async importFrom(provider) {
        if (provider === 'opencode')
            throw new Error('opencode free is anonymous — no import');
