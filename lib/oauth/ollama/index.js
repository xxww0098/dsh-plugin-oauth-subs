...
function ollamaModel(id, name, contextWindow = OLLAMA_DEFAULT_CONTEXT, maxTokens = OLLAMA_DEFAULT_MAX_TOKENS, input = OLLAMA_TEXT_INPUT) {
    return { id, name, contextWindow, maxTokens, input, reasoningEfforts: OLLAMA_REASONING };
}
...
export const OLLAMA_MODELS = Object.freeze([
    ...
    ollamaModel('glm-5.3', 'GLM-5.3', 200_000),
    ollamaModel('kimi-k2.6', 'Kimi K2.6', 256_000),
    ollamaModel('minimax-m3', 'MiniMax M3', 200_000),
    ollamaModel('gemma4:31b', 'Gemma 4 31B', 128_000, OLLAMA_DEFAULT_MAX_TOKENS, OLLAMA_VISION_INPUT),
...
export function inferOllamaContextWindow(id) {
    const text = String(id ?? '').toLowerCase();
    if (/qwen3\.5/.test(text))
        return 262_144;
    if (/kimi/.test(text))
        return 256_000;
    if (/mistral-large|minimax|glm-5/.test(text))
        return 200_000;
