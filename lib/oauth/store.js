...
export const PROVIDER_IDS = Object.freeze(['codex', 'grok', 'glm', 'kiro', 'antigravity', 'cursor', 'ollama', 'kimi', 'copilot', 'devin']);
...
const SESSION_CREDENTIAL_KEYS = new Set([
    'accessToken', 'refreshToken', 'idToken', 'expiresAt', 'sessionToken', 'tokenType',
]);
function mergeSavedSession(existing, session) {
    ...
    for (const [key, value] of Object.entries(existing)) {
        if (SESSION_CREDENTIAL_KEYS.has(key))
            continue;
