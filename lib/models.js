...
import { isUltraBase, isUltraKey } from './ultra-mode.js'
import {
...
export function isOptInKey(key) {
  return isLargeContextKey(key) || isUltraKey(key)
}
...
export function withPickerVariants(models) {
  ...
  for (const model of models) {
    ...
    if (isUltraBase(model.id) && !String(model.id).endsWith('-ultra')) {
      out.push({ ...model, id: `${model.id}-ultra`, name: `${model.name} Ultra` })
    }
 ...
 * Default is all-on except `-900k` and `-ultra` (opt-in; they burn quota).
