...
function usageBag(value) {
    if (!value || typeof value !== 'object')
        return undefined;
    const usedPercent = clampPct(value.usagePercent
        ?? value.usedPercent
        ?? value.percentUsed
        ?? value.percent
        ?? value.usage_percent
...
function windowRow(kind, bag, now) {
    ...
    if (bag.status !== undefined)
        row.status = bag.status;
...
export function parseOpencodeGoUsage(text, now = Date.now()) {
    ...
    if (!bags) {
        ...
        if (rolling.usedPercent === undefined)
...
async function fetchServerText({ cookieHeader, method, args = undefined, fetchFn, signal }) {
...
async function fetchConsoleJson({ path, workspaceId, cookieHeader, fetchFn, signal }) {
    const headers = {
        Cookie: cookieHeader,
        'User-Agent': OPENCODE_GO_PAGE_UA,
    ...
    const text = await response.text();
    if (!response.ok)
        throw new Error(`OpenCode Go console HTTP ${response.status}${serverMessage(text)}`);
    try {
...
function meterRow(kind, meter, fallbackResetAt) {
    ...
    let usedPercent = clampPct(meter.usagePercent ?? meter.usedPercent ?? meter.percentUsed ?? meter.percent);
...
export function parseOpencodeGoConsoleBilling(payload) {
    if (!payload || typeof payload !== 'object')
        return {};
    const result = {};
    if (typeof payload.billingMode === 'string')
        result.useBalance = payload.billingMode === 'prepaid';
...
async function consoleFirst(cookieHeader, consoleFn, legacyFn) {
    try {
        ...
        if (consoleError?.code === 'no_subscription')
            throw consoleError;
        if (!cookieHas(cookieHeader, LEGACY_COOKIE_NAMES))
            throw consoleError;
        try {
...
async function resolveOpencodeGoWorkspace(cookieHeader, { fetchFn, signal } = {}) {
    return consoleFirst(cookieHeader, async () => {
...
async function fetchLegacyOpencodeGoQuota(workspaceId, cookieHeader, { fetchFn, signal, now }) {
...
async function fetchConsoleOpencodeGoQuota(workspaceId, workspaceName, cookieHeader, { fetchFn, signal, now }) {
    ...
    try {
        ...
        if (error?.code !== 'no_subscription' || !(billing.balance > 0))
            throw error;
        usage = { rows: [] };
    }
    return {
...
async function fetchKeyOpencodeGoQuota(apiKey, { fetchFn, signal, now }) {
    const response = await fetchFn(OPENCODE_GO_ORIGIN + '/zen/go/v1/usage', {
        headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' },
        signal,
    });
    if (!response.ok)
        throw new Error(`OpenCode Go key usage HTTP ${response.status}`);
...
export async function fetchOpencodeGoQuota(entry, options = {}) {
    ...
    if (!cookieHeader && !apiKey)
        throw new Error('OpenCode Go API key or cookie is missing');
    const ac = new AbortController();
    const timer = setTimeout(() => ac.abort(), timeoutMs);
    try {
        const ctx = { fetchFn, signal: ac.signal, now };
        if (!cookieHeader)
            return fetchKeyOpencodeGoQuota(apiKey, ctx);
        try {
            ...
            return await consoleFirst(cookieHeader, () => fetchConsoleOpencodeGoQuota(workspace.id, workspace.name, cookieHeader, ctx), () => fetchLegacyOpencodeGoQuota(workspace.id, cookieHeader, ctx));
        }
        catch (error) {
            if (!apiKey)
                throw error;
