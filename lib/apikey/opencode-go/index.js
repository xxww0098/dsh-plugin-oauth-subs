...
export function normalizeOpencodeGoWorkspaceId(raw) {
    ...
    try {
        ...
        for (const marker of ['workspace', 'console']) {
            ...
            if (typeof candidate === 'string' && WORKSPACE_ID_RE.test(candidate))
                return candidate;
        }
    }
    catch {
...
export function publicOpencodeGoAccount(id, entry, quota, active) {
    ...
    return {
        ...
        account: displayName || email || workspaceId,
        displayName,
        email,
        workspaceId,
        workspaceName: String(entry?.workspaceName ?? '').trim(),
