...
import { isOpencodeGoCookieMask, normalizeOpencodeGoWorkspaceId, parseOpencodeGoCookie, publicOpencodeGo, } from './index.js';
...
export class OpencodeGoStore {
    ...
    async snapshot({ refresh = false } = {}) {
        ...
        if (shouldRefresh)
            await this.refreshQuota().catch(() => undefined);
    ...
    async clear(field) {
        await this.ready;
        if (field === 'cookie')
            this.entry = { ...this.entry, cookieHeader: '' };
        else if (field === 'workspace')
            this.entry = { ...this.entry, workspaceId: '' };
        else
            this.entry = emptyEntry();
        await this.#persist();
