 ...
 */
export function ollamaShowInput(show) {
    if (show == null || typeof show !== 'object' || !Array.isArray(show.capabilities))
        return undefined;
    const vision = show.capabilities.some((item) => String(item ?? '').toLowerCase() === 'vision');
    return vision ? [...OLLAMA_VISION_INPUT] : [...OLLAMA_TEXT_INPUT];
}
export function ollamaInput(id, show) {
    return ollamaShowInput(show) ?? inferOllamaInput(id);
}
...
export const OLLAMA_MODELS = Object.freeze([
    ...
    ollamaModel('qwen3.5:397b', 'Qwen 3.5 397B', 262_144, OLLAMA_VISION_INPUT),
]);
...
export function isOllamaOpaqueAccount(value) {
...
export function parseOllamaMe(value) {
    if (!value || typeof value !== 'object')
        return {};
    const user = value.user && typeof value.user === 'object' ? value.user : value;
    const account = firstTrimmed(user.Email, user.email, user.Name, user.name, user.preferred_username, user.username, value.Email, value.email, value.Name, value.name);
    const planType = firstTrimmed(user.Plan, user.plan, value.Plan, value.plan);
    return {
        ...(account ? { account } : {}),
        ...(planType ? { planType } : {}),
    };
...
export async function resolveOllamaIdentity(session, { fetchFn = fetch, signal } = {}) {
    ...
    try {
        ...
        if (!response.ok)
            return undefined;
        const identity = parseOllamaMe(await response.json());
        return identity.account || identity.planType ? identity : undefined;
    }
    catch {
        return undefined;
    }
