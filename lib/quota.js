/**
 * Subscription quota, ported from cockpit-tools:
...
function resetAtOf(window) {
  const stamp = stampOf(window?.reset_at ?? window?.resetAt)
  if (stamp !== undefined) return stamp
  const after = asNumber(window?.reset_after_seconds ?? window?.resetAfterSeconds)
...
export function parseGrokBilling(billing, { cliUser } = {}) {
  ...
  const subscriptionTier = grokTierFromValue(pickPlanRaw(
