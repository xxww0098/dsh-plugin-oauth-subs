  ...
  factory: (require) => {
    ...
    const COPY = {
      zh: {
        ...
        grokHint: 'Hermes 设备码（默）或 PKCE · 127.0.0.1:56121',
        login: '登录',
        pkce: 'PKCE 登录',
        device: '设备码登录',
        import: '导入本机会话',
        ...
        sync: '同步到模型列表',
        fastTitle: 'Fast',
        fastHint: 'GPT 旗舰（不含 Codex）和 Grok 4.6 走 Priority Processing，更快但用量更高。Codex 系列不支持。',
        fastOn: '已开启',
        fastOff: '已关闭',
        loggedOut: '未登录',
        loggedIn: '已登录',
        ...
        plan: '套餐',
        modelsTitle: '模型',
        modelsHint: '勾选要同步到 DeepSeek Harness 的模型。关闭后从模型列表移除。未登录的系列会在登录后生效。',
        modelsOn: '已开启 {n}',
        modelsAll: '全选',
        modelsNone: '全关',
      ...
      en: {
        ...
        grokHint: 'Hermes device-code (default) or PKCE · 127.0.0.1:56121',
        login: 'Sign in',
        pkce: 'PKCE sign-in',
        ...
        fastTitle: 'Fast',
        fastHint: 'Priority Processing for GPT flagships (not Codex) and Grok 4.6. Faster, higher usage. Codex-series models ignore this.',
        fastOn: 'On',
        fastOff: 'Off',
        ...
        plan: 'Plan',
        modelsTitle: 'Models',
        modelsHint: 'Enable models to sync into DeepSeek Harness. Disabled models leave the picker. Families that are signed out apply on the next sign-in.',
        modelsOn: '{n} on',
        modelsAll: 'All on',
        modelsNone: 'All off',
    ...
    function formatReset(resetAt, t) {
      if (typeof resetAt !== 'number' || resetAt <= 0) return ''
      const delta = resetAt - Date.now()
      if (delta <= 0) return t.resetSoon
      const minutes = Math.max(1, Math.round(delta / 60_000))
      if (minutes < 60) return fill(t.resetMinutes, minutes)
      const hours = Math.round(minutes / 60)
      if (hours < 48) return fill(t.resetHours, hours)
      try {
    ...
    function ProviderCard({ t, id, title, hint, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onRefreshQuota, onResetQuota }) {
      ...
      return h('section', {
        ...
        h('header', { style: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' } },
    ...
    function ModelRow({ t, model, onToggle }) {
      return h('label', {
        ...
        h('span', { style: { flex: '1 1 auto', fontSize: 13 } }, model.name),
        model.fast && h('span', {
          style: {
            fontSize: 10,
    ...
    function SettingsSection({ rpc, close: _close }) {
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 720 } },
        ...
        h('div', {
          ...
          h('div', null,
            h('h3', { style: { margin: 0, fontSize: 16, fontWeight: 600 } }, t.fastTitle),
            h('p', { style: { margin: '4px 0 0', opacity: 0.62, fontSize: 12 } }, t.fastHint),
          ),
          h('button', {
            type: 'button',
            onClick: () => run('fast', { on: !snap?.fast?.on }),
            style: { minHeight: 40 },
          }, snap?.fast?.on ? t.fastOn : t.fastOff),
        ),
        h(ModelPicker, {
          t,
        ...
        h('button', { type: 'button', onClick: () => run('sync', {}), style: { alignSelf: 'flex-start', minHeight: 40 } }, t.sync),
      )
    }
