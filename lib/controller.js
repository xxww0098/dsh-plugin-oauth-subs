/**
 * Auth controller behind the Settings page and /v0/oauth management API.
 * Mirrors Hermes Agent's login surface: Codex PKCE (+ paste callback +
 * import), Grok device-code (primary) + PKCE fallback.
 */
...
export class AuthController {
  constructor({ authPath, prefix, origin, settings, grokLogin = 'device', onAuthChanged, fast, models, fetchFn = fetch, quotaTtlMs }) {
...
export async function dispatchManage(controller, method, path, body) {
  const rest = path.replace(/^\/v0\/oauth/, '') || '/'
  try {
    if (method === 'GET' && rest === '/status') {
      return { status: 200, body: await controller.snapshot() }
    }
    if (method === 'POST' && rest === '/login') {
      return { status: 200, body: await controller.login(body.provider, body.mode) }
    }
    if (method === 'POST' && rest === '/manual') {
      await controller.manual(body.provider, body.input)
      return { status: 200, body: { ok: true } }
    }
    if (method === 'POST' && rest === '/cancel') {
      await controller.cancel(body.provider)
      return { status: 200, body: { ok: true } }
    }
    if (method === 'POST' && rest === '/logout') {
      await controller.logout(body.provider)
      return { status: 200, body: { ok: true } }
    }
    if (method === 'POST' && rest === '/import') {
      return { status: 200, body: await controller.importFrom(body.provider) }
    }
    if (method === 'POST' && rest === '/sync') {
      return { status: 200, body: await controller.sync(body.selected) }
    }
    if (method === 'POST' && rest === '/models') {
      return { status: 200, body: await controller.setModels(body) }
    }
    if (method === 'POST' && rest === '/fast') {
      return { status: 200, body: await controller.setFast(body.on) }
    }
    if (method === 'POST' && rest === '/quota') {
      return { status: 200, body: await controller.refreshQuota(body.provider) }
    }
    if (method === 'GET' && rest === '/quota') {
      return { status: 200, body: await controller.refreshQuota(body.provider) }
    }
    if (method === 'POST' && rest === '/quota/reset') {
      return { status: 200, body: await controller.consumeReset(body.provider) }
    }
    return { status: 404, body: { error: `unknown oauth route ${method} ${rest}` } }
  } catch (error) {
