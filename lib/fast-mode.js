 ...
 *
 * DSH has no slash command, so Fast is:
 *   1. a Settings toggle (session default, same as Hermes `/fast on|off`)
...
export function applyFastMode(payload, { defaultOn = false } = {}) {
  ...
  const next = { ...payload }
  const { model, requestedFast } = peelFastSuffix(next.model)
