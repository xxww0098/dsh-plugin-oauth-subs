...
`;
        ...
        // https://unpkg.com/@lobehub/icons-static-svg@1.94.0/icons/{codex,grok,zai,kiro,antigravity,cursor,ollama,kimi,copilot,devin,github}.svg
        const TAB_ICONS = {
        ...
        function QuotaMeter({ t, remainingPercent, amount, label, reset, onToggleAmount }) {
            ...
            return h('div', { className: 'osubs-qmeter' }, h('div', { className: 'osubs-qrow-head' }, h('span', { style: { color: 'var(--osubs-muted)' } }, label), h('span', { style: { color, fontWeight: 500 } }, amount
                ...
                : '', caption)), reset && h('span', { className: 'osubs-qreset' }, reset), remainingPercent !== undefined && h(RemainingBar, { remainingPercent, tone }));
        }
        function QuotaRow({ t, row, family, exactUnits, onToggleUnits }) {
            ...
            }), row.status && row.status !== 'ok' && h('span', { className: 'osubs-tag osubs-tag--warn' }, row.status), row.note && h('span', { className: 'osubs-note' }, row.note));
        }
        ...
        function QuotaBlock({ t, quota, onRefresh, onReset, family }) {
            ...
            }, `${t.quotaFailed}${quota.error ? ` · ${formatQuotaError(quota.error)}` : ''}`), quota.status === 'ready' && !hasUsage && h('p', { className: 'osubs-hint' }, t.quotaUnknown), renderQuotaRows(rows, t, family, units), h(QuotaResetBox, { t, quota, onReset }));
        }
        ...
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey, onGoSave }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, title), h('span', {
                ...
                target: '_blank',
                rel: 'noreferrer',
            }, t.openUrl), busy && h(Button, { onClick: () => onCancel(id), label: t.cancel }), id !== 'glm' && id !== 'kiro' && id !== 'ollama' && id !== 'opencode-go' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                ...
                className: 'osubs-login',
                onClick: () => setShowKey((open) => !open),
            }, h('span', null, t.copilotLoginApiKey)), id === 'devin' && h('button', {
                type: 'button',
                className: 'osubs-login',
                onClick: () => setShowKey((open) => !open),
            }, h('span', null, t.devinLoginApiKey)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => { onImport(id); closeAdd(); },
            }, h('span', null, id === 'cursor' ? t.cursorImport : id === 'kimi' ? t.kimiImport : id === 'copilot' ? t.copilotImport : id === 'devin' ? t.devinImport : t.import))), id === 'kimi' && showKey && !busy && h('form', {
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.copilotKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.copilotKeyGo }))), id === 'devin' && showKey && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
                    onUseKey(id, apiKey);
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.devinKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.devinKeyGo }))), id === 'ollama' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                className: 'osubs-login',
                onClick: () => setShowKey((open) => !open),
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.ollamaKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.ollamaKeyGo }))), id === 'opencode-go' && !busy && h('p', { className: 'osubs-hint' }, t.opencodeGoHint), id === 'opencode-go' && !busy && h('form', {
                className: 'osubs-fields',
