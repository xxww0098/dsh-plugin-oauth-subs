...
`;
        ...
        function AboutPanel({ t, local, update, busy, applying, onCheck, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, autoUpdate, onAutoUpdate, }) {
            ...
            })))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshPublishedAt), h('span', null, dshTag?.publishedAt || '—')), dshHint && h('p', { className: 'osubs-hint' + (dshTone ? ' ' + dshTone : '') }, dshHint), dshApply && h('p', { className: 'osubs-hint' + (dshApplyTone ? ' ' + dshApplyTone : '') }, dshApply))));
