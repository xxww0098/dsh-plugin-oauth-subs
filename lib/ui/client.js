...
`;
        ...
        // https://unpkg.com/@lobehub/icons-static-svg@1.94.0/icons/{codex,grok,zai,kiro,antigravity,cursor,github}.svg
        const TAB_ICONS = {
        ...
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, title), h('span', {
                ...
                target: '_blank',
                rel: 'noreferrer',
            }, t.openUrl), busy && h(Button, { onClick: () => onCancel(id), label: t.cancel }), id !== 'glm' && id !== 'kiro' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
            ...
            }, h('span', null, id === 'cursor' ? t.cursorImport : t.import))), id === 'glm' && !busy && h('div', { className: 'osubs-glm-logins' }, h('button', {
                type: 'button',
        ...
        function SettingsSection({ rpc, close: _close }) {
            ...
            const run = async (method, payload) => {
                try {
                    ...
                    setError(message === 'cursor-import-empty' ? t.cursorImportEmpty : message);
                }
            };
