...
`;
        ...
        // https://unpkg.com/@lobehub/icons-static-svg@1.94.0/icons/{codex,grok,zai,kiro,antigravity,github}.svg
        const TAB_ICONS = {
            ...
            cursor: { d: 'M11.925 24l10.425-6-10.425-6L1.5 18l10.425 6zm10.725-7.2V7.2L12.225 1.2v9.6l10.425 6zM11.925 12.9V3.3L1.5 9.3v9.6l10.425-6z' },
        ...
        function QuotaRow({ t, row, family }) {
            ...
            return h('div', { className: 'osubs-qrow' }, h('div', { className: 'osubs-qrow-head' }, h('span', { style: { color: 'var(--osubs-muted)' } }, rowLabel(row, t, family)), h('span', { style: { color, fontWeight: 500 } }, amount ? `${amount} · ` : '', remaining === undefined ? '' : fill(t.leftPercent, remaining))), remaining !== undefined && h('div', { className: 'osubs-bar' }, h('i', {
                style: {
                    background: color,
        ...
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { style: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' } }, h('h3', { style: { fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' } }, title), h('span', {
                ...
                target: '_blank',
                rel: 'noreferrer',
            }, t.openUrl), h('div', { className: 'osubs-actions' }, id !== 'glm' && id !== 'kiro' && !busy && h(Button, { variant: 'primary', onClick: () => onLogin(id), label: loggedIn ? t.addAccount : t.login }), id === 'grok' && !busy && h(Button, { onClick: () => onLogin(id, 'pkce'), label: t.pkce }), !busy && h(Button, { onClick: () => onImport(id), label: id === 'cursor' ? t.cursorImport : t.import }), busy && h(Button, { onClick: () => onCancel(id), label: t.cancel })), id === 'glm' && !busy && h('div', { className: 'osubs-glm-logins' }, h('button', {
                type: 'button',
            ...
            }, h('span', null, t.kiroLoginRefresh))), id === 'kiro' && showIdc && !busy && h('form', {
