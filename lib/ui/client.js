    ...
    factory: (require) => {
        ...
        const COPY = {
            zh: {
                ...
                dshStatusError: '检查失败',
                dshUpdateInstalled: '已更新完毕。请在终端重启 dsh web 后生效。',
            ...
            en: {
                ...
                dshUpdateInstalled: 'Update installed. Restart dsh web in your terminal to apply.',
...
`;
        ...
        function fresherAboutVersion(left, right) {
            ...
            if (a && b) {
                if (a.major !== b.major)
                    return a.major > b.major ? a.raw : b.raw;
                if (a.minor !== b.minor)
                    return a.minor > b.minor ? a.raw : b.raw;
                if (a.patch !== b.patch)
        ...
        function AboutPanel({ t, local, update, busy, applying, onCheck, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, }) {
            ...
            const dshTag = dshUpdate?.latestTag;
            const dshNpm = dshUpdate?.npm;
            ...
            const dshBtnLabel = dshBusy
                ...
                : (dshUpdate?.canUpdate ? t.dshUpdateAction : t.dshCheckUpdate);
            const dshCard = h('section', { className: 'osubs-card' }, h('header', { style: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' } }, h('h3', { style: { fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' } }, t.dshTitle), h(Button, { size: 'sm', onClick: onDshCheck, disabled: dshBusy, label: dshBtnLabel })), h('div', { className: 'osubs-about' }, h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshRepo), h('a', { className: 'osubs-link', href: `${dshRepo}/tags`, target: '_blank', rel: 'noreferrer' }, dshSlug)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshInstalled), h('span', null, dshVersion)), dshTag?.tag && h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshLatestTag), h('a', { className: 'osubs-link', href: dshTag.url, target: '_blank', rel: 'noreferrer' }, dshTag.tag)), dshNpm?.version && h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshNpmVersion), h('span', null, dshNpm.version)), dshTag?.publishedAt && h('p', { className: 'osubs-note' }, fill(t.dshTagPublished, dshTag.publishedAt)), dshHint && h('p', { className: `osubs-hint${dshTone ? ` ${dshTone}` : ''}` }, dshHint), dshApply && h('p', { className: `osubs-hint${dshApplyTone ? ` ${dshApplyTone}` : ''}` }, dshApply))));
        ...
        function SettingsSection({ rpc, close: _close }) {
            ...
            const checkDshUpdate = async (apply = false) => {
                setDshBusy(true);
                if (apply)
                    setDshApplying(true);
                try {
                    await run('dshUpdate', { apply });
                }
                finally {
                    setDshBusy(false);
            ...
            return h('div', { className: 'osubs' }, h('div', { className: 'osubs-nav', role: 'tablist' }, h('div', { className: 'osubs-tabs' }, h(Tab, { id: 'codex', label: t.codexTitle, current: tab, onSelect: setTab, icon: 'codex' }), h(Tab, { id: 'grok', label: t.grokTitle, current: tab, onSelect: setTab, icon: 'grok' }), h(Tab, { id: 'glm', label: t.glmTitle, current: tab, onSelect: setTab, icon: 'zai' }), h(Tab, { id: 'kiro', label: t.kiroTitle, current: tab, onSelect: setTab, icon: 'kiro' }), h(Tab, { id: 'antigravity', label: t.antigravityTitle, current: tab, onSelect: setTab, icon: 'antigravity' }), h(Tab, { id: 'cursor', label: t.cursorTitle, current: tab, onSelect: setTab, icon: 'cursor' }), h(Tab, { id: 'ollama', label: t.ollamaTitle, current: tab, onSelect: setTab, icon: 'ollama' }), h(Tab, { id: 'kimi', label: t.kimiTitle, current: tab, onSelect: setTab, icon: 'kimi' }), h(Tab, { id: 'copilot', label: t.copilotTitle, current: tab, onSelect: setTab, icon: 'copilot' })), h('div', { className: 'osubs-tabs-util' }, h(Tab, { id: 'models', label: t.modelsTitle, current: tab, onSelect: setTab, icon: 'models' }), h(Tab, { id: 'about', label: t.aboutTitle, current: tab, onSelect: setTab, icon: 'github' }))), h('div', { className: 'osubs-pane' }, error && h('p', { className: 'osubs-hint osubs-bad' }, error), tab === 'codex' && card('codex', t.codexTitle), tab === 'grok' && card('grok', t.grokTitle), tab === 'glm' && card('glm', t.glmTitle), tab === 'kiro' && card('kiro', t.kiroTitle), tab === 'antigravity' && card('antigravity', t.antigravityTitle), tab === 'cursor' && card('cursor', t.cursorTitle), tab === 'ollama' && card('ollama', t.ollamaTitle), tab === 'kimi' && card('kimi', t.kimiTitle), tab === 'copilot' && card('copilot', t.copilotTitle), tab === 'models' && h(ModelPicker, {
                ...
                dshBusy,
                dshApplying,
                onDshCheck: () => checkDshUpdate(true),
            })));
        }
        function apply(ctx) {
