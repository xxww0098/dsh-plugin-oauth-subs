...
`;
        ...
        function QuotaMeter({ t, remainingPercent, amount, label }) {
            ...
            return h('div', { className: 'osubs-qmeter' }, h('div', { className: 'osubs-qrow-head' }, h('span', { style: { color: 'var(--osubs-muted)' } }, label), h('span', { style: { color, fontWeight: 500 } }, amount ? `${amount} · ` : '', caption)), remainingPercent !== undefined && h(RemainingBar, { remainingPercent, tone }));
        }
        function QuotaRow({ t, row, family }) {
            ...
            }), reset && h('span', { className: 'osubs-note' }, reset), row.note && h('span', { className: 'osubs-note' }, row.note));
        }
        ...
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey }) {
            ...
            }, h('span', null, t.opencodeEnable))), id === 'opencode' && !busy && h('p', { className: 'osubs-hint' }, t.opencodeEnableHint), id === 'glm' && !busy && h('div', { className: 'osubs-glm-logins' }, h('button', {
                type: 'button',
