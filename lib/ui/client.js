...
`;
        ...
        function ModelFamily({ t, group, onToggle, onFamily }) {
            ...
            return h('div', { className: 'osubs-family', style: { opacity: group.loggedIn ? 1 : 0.72 } }, h('div', { className: 'osubs-family-head' }, h('div', { style: { display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' } }, h('h4', { style: { fontSize: 13, fontWeight: 600 } }, group.displayName), h('span', { className: 'osubs-note' }, fill(t.modelsOn, `${enabledCount} / ${models.length}`)), !group.loggedIn && h('span', { className: 'osubs-note' }, `· ${t.modelsNeedLogin}`)), h('div', { className: 'osubs-seg' }, h(Button, { size: 'sm', onClick: () => onFamily(group.family, true), label: t.modelsAll }), h(Button, { size: 'sm', onClick: () => onFamily(group.family, false), label: t.modelsNone }))), h('div', { className: 'osubs-models' }, models.map((model) => h(ModelRow, { t, model, onToggle, key: model.key }))));
        }
        ...
        function SettingsSection({ rpc, close: _close }) {
            ...
            }, t.aboutTitle)), tab === 'codex' && card('codex', t.codexTitle), tab === 'grok' && card('grok', t.grokTitle), tab === 'models' && h(ModelPicker, {
                t,
