...
`;
        ...
        function AboutPanel({ t, local, update, busy, applying, onCheck, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, autoUpdate, onAutoUpdate, }) {
            ...
            }))), h(Button, { size: 'sm', mark: update?.status === 'update', onClick: onCheck, disabled: busy, label: busy ? (applying ? t.updateInstalling : t.checking) : t.checkUpdate }))), h('div', { className: 'osubs-about' }, h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.repo), h('a', { className: 'osubs-link', href: repo, target: '_blank', rel: 'noreferrer' }, slug)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.installed), h('span', null, version)), disk && disk !== version && h('div', { className: 'osubs-kv-row' }, h('span', null, t.onDisk), h('span', null, disk)), stale && loaded && h('div', { className: 'osubs-kv-row' }, h('span', null, t.loadedFrom), h('span', { className: 'osubs-note', title: loaded }, shortPath(loaded))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.latest), h('div', { className: 'osubs-kv-value' }, aboutLink(latest?.url, latest?.tag), Boolean(latest?.publishedAt) && h('span', { className: 'osubs-note' }, latest.publishedAt))), update?.status && update.status !== 'update' && h('p', { className: `osubs-hint${tone ? ` ${tone}` : ''}` }, statusLabel(t, update)), stale && disk && h('p', { className: 'osubs-hint osubs-warn' }, fill(t.updateStaleProcess, disk)), apply && h('p', { className: `osubs-hint${applyTone ? ` ${applyTone}` : ''}` }, apply))));
