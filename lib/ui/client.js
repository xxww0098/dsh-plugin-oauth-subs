...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        function formatPlanLabel(raw, family) {
            ...
            if (family === 'opencode') {
                if (slug === 'go' || compact === 'go' || slug === 'free' || compact === 'free' || slug === 'go_free' || compact === 'gofree')
                    return 'Go Free';
            }
            if (family === 'copilot') {
        ...
        function identityOf(row, family) {
            ...
            if (family === 'opencode')
                return account && !isOpencodeOpaqueIdentity(account) ? account : '';
...
`;
        ...
        // https://unpkg.com/@lobehub/icons-static-svg@1.94.0/icons/{codex,grok,zai,kiro,antigravity,cursor,ollama,kimi,opencode,copilot,github}.svg
        const TAB_ICONS = {
        ...
        function QuotaBlock({ t, quota, onRefresh, onReset, family }) {
            ...
            }, `${t.quotaFailed}${quota.error ? ` · ${formatQuotaError(quota.error)}` : ''}`), quota.status === 'ready' && !hasUsage && family !== 'opencode' && h('p', { className: 'osubs-hint' }, t.quotaUnknown), renderQuotaRows(rows, t, family), h(QuotaResetBox, { t, quota, onReset }));
        }
        ...
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, title), h('span', {
                ...
                target: '_blank',
                rel: 'noreferrer',
            }, t.openUrl), busy && h(Button, { onClick: () => onCancel(id), label: t.cancel }), id !== 'glm' && id !== 'kiro' && id !== 'ollama' && id !== 'opencode' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.ollamaKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.ollamaKeyGo }))), id === 'opencode' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                className: 'osubs-login',
                onClick: () => setShowKey((open) => !open),
            }, h('span', null, t.opencodeLoginApiKey)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => { onImport(id); closeAdd(); },
            }, h('span', null, t.opencodeImport))), id === 'opencode' && showKey && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
                    onUseKey(id, apiKey);
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.opencodeKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.opencodeKeyGo }))), id === 'glm' && !busy && h('div', { className: 'osubs-glm-logins' }, h('button', {
                type: 'button',
        ...
        function ModelFamily({ t, group, onToggle, onFamily }) {
            ...
            return h('div', { className: 'osubs-family', style: { opacity: locked ? 0.72 : 1 } }, h('div', { className: 'osubs-family-head' }, h('div', { style: { display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' } }, h('h4', { style: { fontSize: 13, fontWeight: 600 } }, group.displayName), h('span', { className: 'osubs-note' }, fill(t.modelsOn, `${enabledCount} / ${models.length}`)), locked && h('span', { className: 'osubs-note' }, `· ${t.modelsNeedLogin}`)), h('div', { className: 'osubs-seg' }, h(Button, { size: 'sm', disabled: locked, onClick: () => onFamily(group.family, true), label: t.modelsAll }), h(Button, { size: 'sm', disabled: locked, onClick: () => onFamily(group.family, false), label: t.modelsNone }))), h('div', { className: 'osubs-models' }, models.map((model) => h(ModelRow, { t, model, onToggle, locked, key: model.key }))));
        }
