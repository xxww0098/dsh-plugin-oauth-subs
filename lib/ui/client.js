...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        const DSH_HOST_VERSION_STAMP = "0.1.7-rc.2";
...
`;
        ...
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey, onGoSave, onDshRestart, dshRestarting }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, title), h('span', {
                ...
                onClick: () => onLogin(id),
            }, h('span', null, id === 'grok' || id === 'kimi' || id === 'copilot' || id === 'cline' ? t.device : loggedIn ? t.addAccount : t.login)), id === 'grok' && h('button', {
                type: 'button',
                className: 'osubs-login',
            ...
            }, h('span', null, id === 'cursor' ? t.cursorImport : id === 'kimi' ? t.kimiImport : id === 'copilot' ? t.copilotImport : id === 'devin' ? t.devinImport : id === 'cline' ? t.clineImport : t.import))), id === 'kimi' && showKey && !busy && h('form', {
            ...
            }), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', disabled: goBusy, label: t.opencodeGoSave }))), id === 'opencode-go' && goMessage && (goMessage === t.opencodeGoHostStale
                ? h(RestartHint, { t, text: goMessage, tone: 'osubs-bad', busy: dshRestarting, onRestart: onDshRestart })
                : h('p', { className: 'osubs-hint osubs-bad' }, goMessage)), id === 'glm' && !busy && h('div', { className: 'osubs-glm-logins' }, h('button', {
                type: 'button',
         ...
         */
        async function requestDshRestart(rpc) {
            try {
        ...
        function RestartHint({ t, text, tone, busy, onRestart }) {
            return h('p', { className: 'osubs-hint' + (tone ? ' ' + tone : '') }, text + ' ', h('button', {
                type: 'button',
        ...
        function VersionBand({ currentLabel, latestLabel, current, latest, latestChips, busy, cta }) {
            return h('div', { className: 'osubs-ver' + (busy ? ' osubs-ver--busy' : '') }, h('div', { className: 'osubs-ver-cell' }, h('span', { className: 'osubs-note' }, currentLabel), h('span', { className: 'osubs-ver-num' }, current || '—')), h('span', { className: 'osubs-ver-arrow' }, h(IconArrow)), h('div', { className: 'osubs-ver-cell' }, h('span', { className: 'osubs-note' }, latestLabel), h('span', { className: 'osubs-ver-num' + (latest ? ' osubs-ver-num--next' : '') }, latest || '—'), ...(Array.isArray(latestChips) ? latestChips : [])), cta && h('span', { className: 'osubs-ver-cta' }, cta));
        }
        ...
        function autoRunText(t, entry) {
            ...
            if (entry.status === 'installed')
                return fill(t.autoRunInstalled, entry.version || '');
            if (entry.status === 'current')
                return t.autoRunCurrent;
            if (entry.status === 'update')
                return fill(t.autoRunUpdate, entry.version || '');
            if (entry.status === 'failed')
                return t.autoRunFailed;
            return t.autoRunUnknown;
        }
        function AboutPanel({ t, local, update, busy, applying, onCheck, onApply, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, autoUpdate, autoState, onAutoUpdate, onDshRestart, dshRestarting, }) {
            ...
            const pluginCta = update?.status === 'update' && latestTag
                ...
                : null;
            const pluginCard = h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('div', { className: 'osubs-head-main' }, h('h3', { className: 'osubs-card-title' }, t.pluginAboutTitle || t.aboutTitle), pluginPill), h('div', { className: 'osubs-about-actions' }, h(Button, {
                size: 'sm',
                onClick: onCheck,
                disabled: busy,
                label: busy ? (applying ? t.updateInstalling : t.checking) : t.checkUpdate,
            }))), h('div', { className: 'osubs-about' }, h(VersionBand, {
                ...
                busy: applying,
                cta: pluginCta,
            }), h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.repo), h('a', { className: 'osubs-link osubs-link--icon', href: repo, target: '_blank', rel: 'noreferrer' }, h(TabIcon, { name: 'github' }), slug)), disk && disk !== version && h('div', { className: 'osubs-kv-row' }, h('span', null, t.onDisk), h('span', { className: 'osubs-mono' }, disk)), stale && loaded && h('div', { className: 'osubs-kv-row' }, h('span', null, t.loadedFrom), h('span', { className: 'osubs-note', title: loaded }, shortPath(loaded))), Boolean(latest?.publishedAt) && h('div', { className: 'osubs-kv-row' }, h('span', null, t.publishedAt), h('span', { className: 'osubs-note' }, latest.publishedAt)), h(AutoUpdateRow, {
                t,
                note: autoNote('plugin'),
                checked: Boolean(autoUpdate?.plugin),
                onChange: (event) => onAutoUpdate({ plugin: event.currentTarget.checked }),
            })), h('div', { className: 'osubs-hints' }, update?.status === 'error' && h('p', { className: 'osubs-hint osubs-bad' }, statusLabel(t, update)), stale && disk && h('p', { className: 'osubs-hint osubs-warn' }, fill(t.updateStaleProcess, disk)), apply && apply === t.updateNeedsRestart
            ...
            const dshCta = dshCanUpdate
                ...
                : null;
            const dshCard = h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('div', { className: 'osubs-head-main' }, h('h3', { className: 'osubs-card-title' }, t.dshTitle), dshPill), h('div', { className: 'osubs-about-actions' }, h(Button, {
                size: 'sm',
                ...
                latestChips: distTagChips(dshNpm?.distTags, dshNpm?.version).map((chip) => h('span', { className: 'osubs-tag osubs-tag--plain', key: chip }, chip)),
                busy: dshApplying,
                cta: dshCta,
            }), h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshRepo), h('a', { className: 'osubs-link osubs-link--icon', href: dshRepo + '/tags', target: '_blank', rel: 'noreferrer' }, h(TabIcon, { name: 'github' }), dshSlug)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshLatestTag), h('div', { className: 'osubs-kv-value' }, Boolean(dshTag?.publishedAt) && h('span', { className: 'osubs-note' }, dshTag.publishedAt), aboutLink(dshTag?.url, dshTag?.tag))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshStableVersion), h('div', { className: 'osubs-kv-value' }, aboutLink(dshNpm?.stable ? 'https://www.npmjs.com/package/' + (dshEffective?.npmPackage || '@deepseek-ai/dsh') + '/v/' + dshNpm.stable : '', dshNpm?.stable, 'osubs-mono'), distTagChips(dshNpm?.distTags, dshNpm?.stable).map((chip) => h('span', { className: 'osubs-tag osubs-tag--plain', key: chip }, chip)))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshPickVersion), h('div', { className: 'osubs-version-pick' }, h('select', {
            ...
            })), h('div', { className: 'osubs-hints' }, dshUpdate?.status === 'error' && h('p', { className: 'osubs-hint osubs-bad' }, dshStatusLabel(t, dshUpdate)), dshApply && h('p', { className: 'osubs-hint' + (dshApplyTone ? ' ' + dshApplyTone : '') }, dshApply), dshRestarting && h('p', { className: 'osubs-hint' }, t.dshRestartHint))));
        ...
        function SettingsSection({ rpc, close: _close }) {
            ...
            useEffect(() => {
                if (tab === 'about') {
                    ...
                    if (!dshBusy)
                        void checkDshUpdate(false, undefined, dshUpdate !== null);
                }
            }, [tab]);
            if (rpc === undefined) {
