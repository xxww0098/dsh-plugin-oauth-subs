...
`;
        ...
        // https://unpkg.com/@lobehub/icons-static-svg@1.94.0/icons/{codex,grok,zai,kiro,antigravity,cursor,ollama,github}.svg
        const TAB_ICONS = {
        ...
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, title), h('span', {
                ...
                onClick: () => onLogin(id),
            }, h('span', null, id === 'grok' ? t.device : loggedIn ? t.addAccount : t.login)), id === 'grok' && h('button', {
                type: 'button',
                className: 'osubs-login',
                onClick: () => onLogin(id, 'pkce'),
            }, h('span', null, t.pkce)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => { onImport(id); closeAdd(); },
            }, h('span', null, id === 'cursor' ? t.cursorImport : t.import))), id === 'ollama' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                className: 'osubs-login',
        ...
        function SettingsSection({ rpc, close: _close }) {
            ...
            const run = async (method, payload) => {
                try {
                    ...
                    setError(message === 'cursor-import-empty' ? t.cursorImportEmpty : message === 'ollama-import-empty' ? t.ollamaImportEmpty : message);
                }
            };
