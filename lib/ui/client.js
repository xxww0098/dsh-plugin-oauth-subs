...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        const { createElement: h, useCallback, useEffect, useState, Fragment } = require('react');
        function tryHost(id) {
            try {
                return require(id);
            }
...
`;
        ...
        function AboutPanel({ t, local, update, busy, applying, onCheck, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, autoUpdate, onAutoUpdate, }) {
            ...
            }))), dshTag?.publishedAt && h('p', { className: 'osubs-note' }, fill(t.dshTagPublished, dshTag.publishedAt)), h('label', { className: 'osubs-auto' }, h('input', {
                type: 'checkbox',
                checked: Boolean(autoUpdate?.dsh),
                disabled: dshBusy,
                onChange: (event) => onAutoUpdate({ dsh: event.currentTarget.checked }),
            }), h('span', null, t.autoUpdate)), dshHint && h('p', { className: 'osubs-hint' + (dshTone ? ' ' + dshTone : '') }, dshHint), dshApply && h('p', { className: 'osubs-hint' + (dshApplyTone ? ' ' + dshApplyTone : '') }, dshApply))));
