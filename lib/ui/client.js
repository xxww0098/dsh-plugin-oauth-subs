...
`;
        ...
        function ModelRow({ t, model, onToggle, locked }) {
            ...
            }), h('span', null, model.name), model.large && h('span', { className: 'osubs-tag' }, t.largeTag), model.fast && h('span', { className: 'osubs-tag' }, t.fastTag));
        }
        function ModelFamily({ t, group, onToggle, onFamily, onOpenFamily }) {
            const models = Array.isArray(group.models) ? group.models : [];
            const enabledCount = models.filter((model) => model.enabled).length;
            const locked = !group.loggedIn;
            return h('div', { className: `osubs-family${locked ? ' osubs-family--locked' : ''}` }, h('div', { className: 'osubs-family-head' }, h('div', { style: { display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' } }, h('h4', { style: { fontSize: 13, fontWeight: 600 } }, group.displayName), h('span', { className: 'osubs-note' }, fill(t.modelsOn, `${enabledCount} / ${models.length}`)), locked && h('span', { className: 'osubs-note' }, `· ${t.modelsNeedLogin}`)), locked
                ? h(Button, { size: 'sm', variant: 'primary', onClick: () => onOpenFamily?.(group.family), label: t.login })
                : h('div', { className: 'osubs-seg' }, h(Button, { size: 'sm', onClick: () => onFamily(group.family, true), label: t.modelsAll }), h(Button, { size: 'sm', onClick: () => onFamily(group.family, false), label: t.modelsNone }))), !locked && h('div', { className: 'osubs-models' }, models.map((model) => h(ModelRow, { t, model, onToggle, locked, key: model.key }))));
        }
        function ModelPicker({ t, catalog, onToggle, onFamily, onOpenFamily }) {
            ...
            if (groups.length === 0)
                return null;
            return h('section', { className: 'osubs-card', style: { padding: '18px 20px 20px', gap: 18 } }, h('header', { style: { display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' } }, h('div', { style: { display: 'flex', flexDirection: 'column', gap: 6, flex: '1 1 320px', maxWidth: '62ch' } }, h('h3', { style: { fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' } }, t.modelsTitle), h('p', { className: 'osubs-hint' }, t.modelsHint)), h('span', { className: 'osubs-badge' }, fill(t.modelsOn, `${enabled} / ${total}`))), groups.map((group) => h(ModelFamily, { t, group, onToggle, onFamily, onOpenFamily, key: group.provider })));
        }
        ...
        async function fetchClientDshLatest() {
            ...
            try {
                ...
                if (npmResp.ok) {
                    ...
                    const version = distTags.latest || distTags.next || distTags.alpha;
                    ...
                    npm = { version, publishedAt, distTags, versions };
                }
            }
            catch { }
        ...
        function AboutPanel({ t, local, update, busy, applying, onCheck, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, autoUpdate, onAutoUpdate, }) {
            ...
            const pluginCard = h('section', { className: 'osubs-card' }, h('header', { style: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' } }, h('h3', { style: { fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' } }, t.pluginAboutTitle || t.aboutTitle), h('div', { className: 'osubs-about-actions' }, h(HoldTip, { label: t.autoUpdate }, h('label', { className: 'osubs-auto' }, h('input', {
                type: 'checkbox',
                checked: Boolean(autoUpdate?.plugin),
                'aria-label': t.autoUpdate,
                onChange: (event) => onAutoUpdate({ plugin: event.currentTarget.checked }),
            }))), h(HoldTip, { label: update?.status === 'update' ? fill(t.willUpdate, latest?.tag || '') : t.checkUpdate }, h(Button, { size: 'sm', mark: update?.status === 'update', onClick: onCheck, disabled: busy, label: busy ? (applying ? t.updateInstalling : t.checking) : t.checkUpdate })))), h('div', { className: 'osubs-about' }, h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.repo), h('a', { className: 'osubs-link', href: repo, target: '_blank', rel: 'noreferrer' }, slug)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.installed), h('span', null, version)), disk && disk !== version && h('div', { className: 'osubs-kv-row' }, h('span', null, t.onDisk), h('span', null, disk)), stale && loaded && h('div', { className: 'osubs-kv-row' }, h('span', null, t.loadedFrom), h('span', { className: 'osubs-note', title: loaded }, shortPath(loaded))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.os), h('span', null, platformLabel(t, host))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.latest), aboutLink(latest?.url, latest?.tag)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.publishedAt), h('span', null, latest?.publishedAt || '—')), update?.status && update.status !== 'update' && h('p', { className: `osubs-hint${tone ? ` ${tone}` : ''}` }, statusLabel(t, update)), stale && disk && h('p', { className: 'osubs-hint osubs-warn' }, fill(t.updateStaleProcess, disk)), apply && h('p', { className: `osubs-hint${applyTone ? ` ${applyTone}` : ''}` }, apply))));
            ...
            const dshCard = h('section', { className: 'osubs-card' }, h('header', { style: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' } }, h('h3', { style: { fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' } }, t.dshTitle), h('div', { className: 'osubs-about-actions' }, h(HoldTip, { label: t.autoUpdate }, h('label', { className: 'osubs-auto' }, h('input', {
                type: 'checkbox',
                checked: Boolean(autoUpdate?.dsh),
                'aria-label': t.autoUpdate,
                onChange: (event) => onAutoUpdate({ dsh: event.currentTarget.checked }),
            }))), h(HoldTip, { label: t.dshCheckUpdate }, h(Button, {
                size: 'sm',
                disabled: dshBusy,
            ...
            })))), h('div', { className: 'osubs-about' }, h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshRepo), h('a', { className: 'osubs-link', href: dshRepo + '/tags', target: '_blank', rel: 'noreferrer' }, dshSlug)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshInstalled), h('span', null, dshVersion)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshLatestTag), h('div', { className: 'osubs-kv-value' }, aboutLink(dshTag?.url, dshTag?.tag), h('span', { className: 'osubs-note' }, dshTag?.publishedAt || '—'))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshNpmVersion), aboutLink(dshNpm?.version ? 'https://www.npmjs.com/package/' + (dshEffective?.npmPackage || '@deepseek-ai/dsh') + '/v/' + dshNpm.version : '', dshNpm?.version)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshPickVersion), h('div', { className: 'osubs-version-pick' }, h('select', {
            ...
            })))), dshHint && dshUpdate?.status !== 'update' && h('p', { className: 'osubs-hint' + (dshTone ? ' ' + dshTone : '') }, dshHint), dshApply && h('p', { className: 'osubs-hint' + (dshApplyTone ? ' ' + dshApplyTone : '') }, dshApply))));
