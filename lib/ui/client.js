...
`;
        ...
        function AboutPanel({ t, local, update, busy, applying, onCheck, onApply, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, autoUpdate, autoState, onAutoUpdate, }) {
            ...
            })), h('div', { className: 'osubs-hints' }, update?.status === 'error' && h('p', { className: 'osubs-hint osubs-bad' }, statusLabel(t, update)), stale && disk && h('p', { className: 'osubs-hint osubs-warn' }, fill(t.updateStaleProcess, disk)), apply && h('p', { className: 'osubs-hint' + (applyTone ? ' ' + applyTone : '') }, apply))));
