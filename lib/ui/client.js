...
`;
        ...
        // https://unpkg.com/@lobehub/icons-static-svg@1.94.0/icons/{codex,grok,zai,kiro,antigravity,cursor,ollama,kimi,copilot,github}.svg
        const TAB_ICONS = {
        ...
        function QuotaMeter({ t, remainingPercent, amount, label, reset }) {
            ...
            return h('div', { className: 'osubs-qmeter' }, h('div', { className: 'osubs-qrow-head' }, h('span', { style: { color: 'var(--osubs-muted)' } }, label), h('span', { style: { color, fontWeight: 500 } }, amount ? `${amount} · ` : '', caption)), reset && h('span', { className: 'osubs-qreset' }, reset), remainingPercent !== undefined && h(RemainingBar, { remainingPercent, tone }));
        }
        ...
        function CenterDialog({ titleId, title, closeLabel, onClose, cardClass, bodyClass, children }) {
            ...
            }, h('div', { className: 'osubs-dsw-head' }, h('h2', { id: titleId, className: 'osubs-dsw-title' }, title), h('button', {
                type: 'button',
        ...
        function renderQuotaRows(rows, t, family) {
            ...
            const flush = () => {
                if (!cluster)
                    return;
                nodes.push(h('div', { className: 'osubs-qcluster', key: cluster.key }, h('div', { className: 'osubs-qgroup' }, antigravityGroupLabel(cluster.title, t)), cluster.rows.map((row) => h(QuotaRow, { t, row, family, key: row.key }))));
                cluster = undefined;
            };
        ...
        function QuotaBlock({ t, quota, onRefresh, onReset, family }) {
            ...
            return h('div', { className: 'osubs-quota' }, h('div', { className: 'osubs-quota-head' }, h('span', { className: 'osubs-eyebrow' }, t.quota), h('div', { className: 'osubs-actions' }, h(Button, { size: 'sm', onClick: onRefresh, label: t.quotaRefresh }))), family === 'glm' && h('p', { className: 'osubs-note' }, t.glmBoostHint), quota.status === 'loading' && rows.length === 0 && h('p', { className: 'osubs-hint' }, t.quotaLoading), quota.status === 'error' && !hasUsage && h('p', {
                className: 'osubs-hint osubs-bad',
                title: quota.error || undefined,
            }, `${t.quotaFailed}${quota.error ? ` · ${formatQuotaError(quota.error)}` : ''}`), quota.status === 'ready' && !hasUsage && h('p', { className: 'osubs-hint' }, t.quotaUnknown), renderQuotaRows(rows, t, family), h(QuotaResetBox, { t, quota, onReset }));
        }
        function AccountCard({ t, id, row, quota, onSwitch, onLogout, onRefreshQuota, onResetQuota }) {
            ...
            }, h('div', { className: 'osubs-acct-head' }, h('div', { className: 'osubs-acct-main' }, h('div', { className: 'osubs-acct-row' }, h('span', { className: 'osubs-mono' }, identityOf(row, id)), planLabel && h('span', { className: 'osubs-tag' }, planLabel), row.active && h('span', { className: 'osubs-tag osubs-tag--on' }, t.inUse), id === 'glm' && row.region && h('span', { className: 'osubs-tag' }, regionLabel(row.region)), id === 'kiro' && row.methodLabel && h('span', { className: 'osubs-tag' }, row.methodLabel), (id === 'cursor' || id === 'ollama' || id === 'kimi' || id === 'copilot') && row.methodLabel && h('span', { className: 'osubs-tag' }, row.methodLabel), id === 'glm' && h('span', { className: 'osubs-tag osubs-tag--plain' }, t.glmBoost))), h('div', { className: 'osubs-actions', onClick: (event) => event.stopPropagation() }, !row.active && h(Button, { size: 'sm', onClick: () => onSwitch(id, row.id), label: t.switchTo }), h(Button, { size: 'sm', onClick: () => onLogout(id, row.id), label: t.logout }))), h('div', { onClick: (event) => event.stopPropagation() }, id === 'antigravity' && row.needsValidation && h('div', { className: 'osubs-verify' }, h('p', { className: 'osubs-hint osubs-warn' }, t.antigravityVerify), row.validationUrl && h(Button, {
                size: 'sm',
        ...
        function ApiKeyPanel({ t, go, onSave, onClear, onRefreshQuota }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, t.opencodeGoTitle), h('span', { className: 'osubs-status' + (configured ? ' osubs-status--on' : '') }, configured ? t.loggedIn : t.loggedOut)), h('p', { className: 'osubs-hint' }, t.opencodeGoHint), h('div', { className: 'osubs-fields' }, h('span', { className: 'osubs-eyebrow' }, t.opencodeGoKey), h('input', {
            ...
            })), h('div', { className: 'osubs-actions' }, h(Button, { variant: 'primary', disabled: busy, label: t.opencodeGoSave, onClick: save }), go?.apiKeySet && h(Button, { size: 'sm', disabled: busy, label: t.opencodeGoClearKey, onClick: () => clear('key') }), go?.cookieSet && h(Button, { size: 'sm', disabled: busy, label: t.opencodeGoClear, onClick: () => clear('cookie') }), go?.workspaceId && h(Button, { size: 'sm', disabled: busy, label: t.opencodeGoClearWorkspace, onClick: () => clear('workspace') })), message && h('p', { className: 'osubs-hint' }, message), h(QuotaBlock, { t, family: 'opencode-go', quota: go?.quota, onRefresh: onRefreshQuota }));
        }
        function ProviderCard({ t, id, title, account, pending, onLogin, onImport, onLogout, onCancel, onManual, onSwitch, onRefreshQuota, onResetQuota, onUseKey }) {
            ...
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, title), h('span', {
                className: `osubs-status${loggedIn ? ' osubs-status--on' : busy ? ' osubs-status--busy' : ''}`,
            }, status)), roster.length > 0 && h('div', { className: 'osubs-accts' }, roster.map((row) => h(AccountCard, {
                t,
                id,
                row,
                quota: row.quota,
                ...
                onResetQuota,
                key: row.id,
            }))), account?.detail && h('p', { className: 'osubs-hint osubs-bad' }, `${t.error}: ${account.detail}`), pending?.userCode && busy && h('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } }, h('span', { className: 'osubs-eyebrow' }, t.userCode), h('code', { style: { fontSize: 20, letterSpacing: '0.14em', fontWeight: 600, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' } }, pending.userCode)), pending?.authorizeUrl && busy && h('a', {
                className: 'osubs-link',
                href: pending.authorizeUrl,
                target: '_blank',
                rel: 'noreferrer',
            }, t.openUrl), h('div', { className: 'osubs-actions' }, h(Button, {
                variant: 'primary',
                onClick: () => setAddOpen(true),
                label: busy ? t.continueAuth : loggedIn ? t.addAccount : t.login,
            }), busy && h(Button, { onClick: () => onCancel(id), label: t.cancel })), addOpen && h(CenterDialog, {
            ...
            }, pending?.userCode && busy && h('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } }, h('span', { className: 'osubs-eyebrow' }, t.userCode), h('code', { style: { fontSize: 20, letterSpacing: '0.14em', fontWeight: 600, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' } }, pending.userCode)), pending?.authorizeUrl && busy && h('a', {
                className: 'osubs-link',
                href: pending.authorizeUrl,
                target: '_blank',
                rel: 'noreferrer',
            }, t.openUrl), busy && h(Button, { onClick: () => onCancel(id), label: t.cancel }), id !== 'glm' && id !== 'kiro' && id !== 'ollama' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                className: 'osubs-login',
                onClick: () => onLogin(id),
            }, h('span', null, id === 'grok' || id === 'kimi' || id === 'copilot' ? t.device : loggedIn ? t.addAccount : t.login)), id === 'grok' && h('button', {
                type: 'button',
                className: 'osubs-login',
            ...
            }, h('span', null, t.kimiLoginApiKey)), id === 'copilot' && h('button', {
                type: 'button',
                className: 'osubs-login',
                onClick: () => setShowKey((open) => !open),
            }, h('span', null, t.copilotLoginApiKey)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => { onImport(id); closeAdd(); },
            }, h('span', null, id === 'cursor' ? t.cursorImport : id === 'kimi' ? t.kimiImport : id === 'copilot' ? t.copilotImport : t.import))), id === 'kimi' && showKey && !busy && h('form', {
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.kimiKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.kimiKeyGo }))), id === 'copilot' && showKey && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
                    onUseKey(id, apiKey);
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.copilotKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.copilotKeyGo }))), id === 'ollama' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                className: 'osubs-login',
                onClick: () => setShowKey((open) => !open),
            }, h('span', null, t.ollamaLoginApiKey)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => { onImport(id); closeAdd(); },
            }, h('span', null, t.ollamaImport))), id === 'ollama' && showKey && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
                    onUseKey(id, apiKey);
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.ollamaKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.ollamaKeyGo }))), id === 'glm' && !busy && h('div', { className: 'osubs-glm-logins' }, h('button', {
                type: 'button',
                ...
                onClick: () => setShowKey((open) => !open),
            }, h('span', null, t.glmLoginApiKey)), h('button', {
                type: 'button',
                className: 'osubs-glm-login osubs-glm-ghost',
                onClick: () => { onImport(id); closeAdd(); },
            }, h('span', null, t.import))), id === 'glm' && showKey && !busy && h('form', {
                onSubmit: (event) => {
                    event.preventDefault();
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.glmKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.glmKeyGo }))), id === 'kiro' && !busy && h('div', { className: 'osubs-logins' }, h('button', {
                type: 'button',
                className: 'osubs-login',
            ...
            }, h('span', null, loggedIn ? t.kiroAddIdc : t.kiroLoginIdc), h('span', { className: 'osubs-tag' }, 'IdC')), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => setShowEntra((open) => !open),
            }, h('span', null, t.kiroLoginEntra)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => setShowKey((open) => !open),
            }, h('span', null, t.kiroLoginApiKey)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => setShowRefresh((open) => !open),
            }, h('span', null, t.kiroLoginRefresh)), h('button', {
                type: 'button',
                className: 'osubs-login osubs-login-ghost',
                onClick: () => { onImport(id); closeAdd(); },
            }, h('span', null, t.import))), id === 'kiro' && showIdc && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.kiroStartUrlHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.kiroStartUrlGo }))), id === 'kiro' && showEntra && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.kiroEntraHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.kiroEntraGo }))), id === 'kiro' && showKey && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
                ...
                autoComplete: 'off',
            }), h('p', { className: 'osubs-hint' }, t.kiroKeyHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.kiroKeyGo }))), id === 'kiro' && showRefresh && !busy && h('form', {
                className: 'osubs-fields',
                onSubmit: (event) => {
                    event.preventDefault();
            ...
            }), h('p', { className: 'osubs-hint' }, t.kiroRefreshHint), h('div', { className: 'osubs-actions' }, h(Button, { type: 'submit', variant: 'primary', label: t.kiroRefreshGo }))), busy && (pending?.mode === 'pkce' || pending?.mode === 'oauth') && h('form', {
                onSubmit: (event) => {
                    event.preventDefault();
                    onManual(id, paste);
                },
        ...
        function ModelRow({ t, model, onToggle, locked }) {
            return h('label', { className: 'osubs-model' }, h('input', {
                ...
                onChange: () => { if (!locked)
                    onToggle(model.key, !model.enabled); },
            }), h('span', null, model.name), model.large && h('span', { className: 'osubs-tag' }, t.largeTag), model.fast && h('span', { className: 'osubs-tag osubs-tag--fast' }, t.fastTag));
        }
        function ModelFamily({ t, group, onToggle, onFamily, onOpenFamily }) {
            const models = Array.isArray(group.models) ? group.models : [];
            const enabledCount = models.filter((model) => model.enabled).length;
            const locked = !group.loggedIn;
            return h('div', { className: 'osubs-family' }, h('div', { className: 'osubs-family-head' }, h('div', { className: 'osubs-family-meta' }, h('h4', { className: 'osubs-family-title' }, group.displayName), h('span', { className: 'osubs-note' }, fill(t.modelsOn, `${enabledCount} / ${models.length}`))), locked
                ? h(Button, { size: 'sm', variant: 'primary', onClick: () => onOpenFamily?.(group.family), label: t.login })
                : h('div', { className: 'osubs-seg' }, h(Button, { size: 'sm', onClick: () => onFamily(group.family, true), label: t.modelsAll }), h(Button, { size: 'sm', onClick: () => onFamily(group.family, false), label: t.modelsNone }))), h('div', { className: 'osubs-models' }, models.map((model) => h(ModelRow, { t, model, onToggle, locked, key: model.key }))));
        }
        function ModelPicker({ t, catalog, onToggle, onFamily, onOpenFamily }) {
            const groups = Array.isArray(catalog) ? catalog : [];
            return h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('h3', { className: 'osubs-card-title' }, t.modelsTitle)), groups.map((group) => h(ModelFamily, { t, group, onToggle, onFamily, onOpenFamily, key: group.provider })));
        }
        ...
        function StatusPill({ tone, icon, label }) {
            ...
            if (tone)
                classes.push(`osubs-pill--${tone}`);
            if (icon)
                classes.push('osubs-pill--icon');
        ...
        function applyLabel(t, update) {
            ...
            if (apply.status === 'missing-dsh')
                return t.updateMissingDsh;
            if (apply.status === 'timeout')
                return t.updateTimeout;
            if (apply.status === 'unchanged')
                return fill(t.updateUnchanged, apply.error || '');
            if (apply.status === 'failed')
                return fill(t.updateFailed, apply.error || '');
            return '';
        }
        function dshStatusLabel(t, dsh) {
            if (!dsh)
                return '';
            if (dsh.status === 'update')
                return fill(t.dshStatusUpdate, dsh.npm?.version || '');
            if (dsh.status === 'github-only')
                return fill(t.dshStatusGithubOnly, dsh.latestTag?.tag || dsh.latestTag?.version || '');
            if (dsh.status === 'current')
                return t.dshStatusCurrent;
            if (dsh.status === 'ahead')
                return t.dshStatusAhead;
            if (dsh.status === 'unknown')
                return '';
            if (dsh.status === 'error')
                return `${t.dshStatusError}${dsh.error ? ` · ${dsh.error}` : ''}`;
            return '';
        }
        function dshApplyLabel(t, dsh) {
            ...
            if (!apply || apply.status === 'none')
                return '';
            if (apply.status === 'installed')
                return fill(t.dshUpdateInstalled, apply.after || dsh?.version || '');
            if (apply.status === 'missing-npm')
                return t.dshUpdateMissingNpm;
            if (apply.status === 'timeout')
                return t.dshUpdateTimeout;
            if (apply.status === 'unchanged' || apply.status === 'installed-unchanged')
                return fill(t.dshUpdateUnchanged, apply.error || apply.after || '');
            if (apply.status === 'failed')
                return fill(t.dshUpdateFailed, apply.error || '');
            return '';
        }
        ...
        function AboutPanel({ t, local, update, busy, applying, onCheck, dshLocal, dshUpdate, dshBusy, dshApplying, onDshCheck, autoUpdate, onAutoUpdate, }) {
                    ...
                    : update?.status === 'ahead'
                        ...
                        : update?.status === 'unknown'
                            ...
                            : update?.status === 'error'
                                ...
                                : null;
            const pluginCard = h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('div', { className: 'osubs-head-main' }, h('h3', { className: 'osubs-card-title' }, t.pluginAboutTitle || t.aboutTitle), pluginPill), h('div', { className: 'osubs-about-actions' }, h(HoldTip, { label: t.autoUpdate }, h('label', { className: 'osubs-auto', title: t.autoUpdate }, h('input', {
                type: 'checkbox',
                checked: Boolean(autoUpdate?.plugin),
                'aria-label': t.autoUpdate,
                onChange: (event) => onAutoUpdate({ plugin: event.currentTarget.checked }),
            }), h('span', { className: 'osubs-auto-track', 'aria-hidden': 'true' }))), h(Button, { size: 'sm', mark: update?.status === 'update', onClick: onCheck, disabled: busy, label: busy ? (applying ? t.updateInstalling : t.checking) : t.checkUpdate }))), h('div', { className: 'osubs-about' }, h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.repo), h('a', { className: 'osubs-link osubs-link--icon', href: repo, target: '_blank', rel: 'noreferrer' }, h(TabIcon, { name: 'github' }), slug)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.installed), h('span', { className: 'osubs-mono' }, version)), disk && disk !== version && h('div', { className: 'osubs-kv-row' }, h('span', null, t.onDisk), h('span', { className: 'osubs-mono' }, disk)), stale && loaded && h('div', { className: 'osubs-kv-row' }, h('span', null, t.loadedFrom), h('span', { className: 'osubs-note', title: loaded }, shortPath(loaded))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.latest), h('div', { className: 'osubs-kv-value' }, aboutLink(latest?.url, latest?.tag, 'osubs-mono'), Boolean(latest?.publishedAt) && h('span', { className: 'osubs-note' }, latest.publishedAt)))), h('div', { className: 'osubs-hints' }, update?.status === 'error' && h('p', { className: 'osubs-hint osubs-bad' }, statusLabel(t, update)), stale && disk && h('p', { className: 'osubs-hint osubs-warn' }, fill(t.updateStaleProcess, disk)), apply && h('p', { className: `osubs-hint${applyTone ? ` ${applyTone}` : ''}` }, apply))));
                    ...
                    : dshUpdate?.status === 'current'
                        ...
                        : dshUpdate?.status === 'ahead'
                            ...
                            : dshUpdate?.status === 'error'
                                ...
                                : null;
            const dshCard = h('section', { className: 'osubs-card' }, h('header', { className: 'osubs-card-head' }, h('div', { className: 'osubs-head-main' }, h('h3', { className: 'osubs-card-title' }, t.dshTitle), dshPill), h('div', { className: 'osubs-about-actions' }, h(HoldTip, { label: t.autoUpdate }, h('label', { className: 'osubs-auto', title: t.autoUpdate }, h('input', {
                type: 'checkbox',
                checked: Boolean(autoUpdate?.dsh),
                'aria-label': t.autoUpdate,
                onChange: (event) => onAutoUpdate({ dsh: event.currentTarget.checked }),
            }), h('span', { className: 'osubs-auto-track', 'aria-hidden': 'true' }))), h(Button, {
                size: 'sm',
                disabled: dshBusy,
            ...
            }))), h('div', { className: 'osubs-about' }, h('div', { className: 'osubs-kv' }, h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshRepo), h('a', { className: 'osubs-link osubs-link--icon', href: dshRepo + '/tags', target: '_blank', rel: 'noreferrer' }, h(TabIcon, { name: 'github' }), dshSlug)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshInstalled), h('span', { className: 'osubs-mono' }, dshVersion)), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshLatestTag), h('div', { className: 'osubs-kv-value' }, Boolean(dshTag?.publishedAt) && h('span', { className: 'osubs-note' }, dshTag.publishedAt), aboutLink(dshTag?.url, dshTag?.tag))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshNpmVersion), h('div', { className: 'osubs-kv-value' }, aboutLink(dshNpm?.version ? 'https://www.npmjs.com/package/' + (dshEffective?.npmPackage || '@deepseek-ai/dsh') + '/v/' + dshNpm.version : '', dshNpm?.version, 'osubs-mono'), distTagChips(dshNpm?.distTags, dshNpm?.version).map((chip) => h('span', { className: 'osubs-tag osubs-tag--plain', key: chip }, chip)))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshStableVersion), h('div', { className: 'osubs-kv-value' }, aboutLink(dshNpm?.stable ? 'https://www.npmjs.com/package/' + (dshEffective?.npmPackage || '@deepseek-ai/dsh') + '/v/' + dshNpm.stable : '', dshNpm?.stable, 'osubs-mono'), distTagChips(dshNpm?.distTags, dshNpm?.stable).map((chip) => h('span', { className: 'osubs-tag osubs-tag--plain', key: chip }, chip)))), h('div', { className: 'osubs-kv-row' }, h('span', null, t.dshPickVersion), h('div', { className: 'osubs-version-pick' }, h('select', {
                ...
                if (!ver)
                    return h('option', { key: '', value: '' }, '—');
                const tags = dshNpm?.distTags || {};
                const marks = Object.keys(tags).filter((key) => tags[key] === ver);
                if (listedLocal === ver || ver === dshVersion)
                    marks.unshift(t.dshOnThisMachine);
                const label = marks.length ? ver + ' (' + marks.join(', ') + ')' : ver;
                return h('option', { key: ver, value: ver }, label);
            })), dshCanApply && h(HoldTip, { label: fill(dshCmp < 0 ? t.willRollback : dshVersion === '—' ? t.willInstall : t.willUpdate, dshChoice) }, h(Button, {
                size: 'sm',
                disabled: dshBusy,
                label: dshApplying ? t.dshUpdating : dshSwitchLabel,
                onClick: () => onDshCheck(true, dshChoice),
            }))))), h('div', { className: 'osubs-hints' }, dshUpdate?.status === 'error' && h('p', { className: 'osubs-hint osubs-bad' }, dshStatusLabel(t, dshUpdate)), dshApply && h('p', { className: 'osubs-hint' + (dshApplyTone ? ' ' + dshApplyTone : '') }, dshApply))));
        ...
        function SettingsSection({ rpc, close: _close }) {
            ...
            const checkDshUpdate = async (apply = false, targetVersion) => {
                setDshBusy(true);
                if (apply)
                    setDshApplying(true);
                try {
                    ...
                    try {
                        ...
                        if (res && typeof res === 'object') {
                            ...
                            if (apply || res.latestTag || res.npm || res.version)
                                return res;
                        }
                    }
                    catch (rpcErr) {
                        const message = rpcErr instanceof Error ? rpcErr.message : String(rpcErr);
                        if (apply && !isUnknownOauthMethod(message))
                            throw rpcErr;
                    }
                    if (!apply || !res) {
            ...
            useEffect(() => {
                ...
                if (!host || autoUpdateSynced.current)
                    return;
            ...
            useEffect(() => {
                if (tab === 'about') {
                    if (update === null && !updateBusy)
                        void checkUpdate(false);
                    if (dshUpdate === null && !dshBusy)
                        void checkDshUpdate(false);
                }
            }, [tab]);
            if (rpc === undefined) {
            ...
            return h('div', { className: 'osubs' }, h('div', { className: 'osubs-nav', role: 'tablist' }, h('div', { className: 'osubs-tabs' }, h(Tab, { id: 'codex', label: t.codexTitle, current: tab, onSelect: setTab, icon: 'codex' }), h(Tab, { id: 'grok', label: t.grokTitle, current: tab, onSelect: setTab, icon: 'grok' }), h(Tab, { id: 'glm', label: t.glmTitle, current: tab, onSelect: setTab, icon: 'zai' }), h(Tab, { id: 'kiro', label: t.kiroTitle, current: tab, onSelect: setTab, icon: 'kiro' }), h(Tab, { id: 'antigravity', label: t.antigravityTitle, current: tab, onSelect: setTab, icon: 'antigravity' }), h(Tab, { id: 'cursor', label: t.cursorTitle, current: tab, onSelect: setTab, icon: 'cursor' }), h(Tab, { id: 'ollama', label: t.ollamaTitle, current: tab, onSelect: setTab, icon: 'ollama' }), h(Tab, { id: 'kimi', label: t.kimiTitle, current: tab, onSelect: setTab, icon: 'kimi' }), h(Tab, { id: 'copilot', label: t.copilotTitle, current: tab, onSelect: setTab, icon: 'copilot' }), h(Tab, { id: 'apikey', label: t.apiKeyTitle, current: tab, onSelect: setTab, icon: 'opencodeGo' })), h('div', { className: 'osubs-tabs-util' }, h(Tab, { id: 'models', label: t.modelsTitle, current: tab, onSelect: setTab, icon: 'models' }), h(Tab, { id: 'about', label: t.aboutTitle, current: tab, onSelect: setTab, icon: 'github' }))), h('div', { className: 'osubs-pane' }, error && h('p', { className: 'osubs-hint osubs-bad' }, error), panel('codex', card('codex', t.codexTitle)), panel('grok', card('grok', t.grokTitle)), panel('glm', card('glm', t.glmTitle)), panel('kiro', card('kiro', t.kiroTitle)), panel('antigravity', card('antigravity', t.antigravityTitle)), panel('cursor', card('cursor', t.cursorTitle)), panel('ollama', card('ollama', t.ollamaTitle)), panel('kimi', card('kimi', t.kimiTitle)), panel('copilot', card('copilot', t.copilotTitle)), panel('apikey', h(ApiKeyPanel, {
                t,
                go: snap?.opencodeGo,
