...
function rewriteFastBody(buffer) {
  if (!buffer.length) return buffer
  try {
    const payload = JSON.parse(buffer.toString('utf8'))
    return Buffer.from(JSON.stringify(applyFastMode(payload)))
  } catch {
    return buffer
  }
}

export function createProxy({ port, bind = '127.0.0.1', apiKey, tokens, fetchFn = fetch, onManage }) {
  let server
...
async function forward(request, response, { url, session, headersOf, fetchFn }) {
  const raw = await readBody(request)
  const body = rewriteFastBody(raw)
