 ...
 *      Kimi Code Plan, OpenCode Go Free, and GitHub Copilot logins
...
async function rememberCredential(ctx, key) {
    ...
    if (!process.env[OAUTH_CREDENTIAL_REF])
        process.env[OAUTH_CREDENTIAL_REF] = key;
}
    ...
    ctx.effect(() => {
        ...
        void (async () => {
            try {
                ...
                if (!closed)
                    ctx.logger?.error?.(`dsh-plugin-oauth-subs: failed to start: ${error.message}`);
            }
        })();
        return () => {
            closed = true;
...
export { OPENCODE_MODELS, OPENCODE_CHAT_URL, OPENCODE_RESPONSES_URL, isOpencodeResponsesModel, opencodeSession, opencodeUpstreamHeaders, } from './oauth/opencode/index.js';
...
export { REPO_URL, REPO_SLUG, installedVersion, fresherVersion, fetchLatest, localUpdateInfo, profileFromBaseUrl, pluginUpdateCommand, runPluginUpdate, applyHostUpdate, } from './utils/update.js';
