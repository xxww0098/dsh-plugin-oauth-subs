...
export function registerRpc(ctx, controller) {
    ...
    const mountFetchRoutes = (connection) => {
        ...
        if (typeof fetchApi?.register !== 'function')
            return false;
    ...
    ctx.inject(['connection', 'webServer'], (scope) => {
        ...
        if (typeof rpc?.handle !== 'function') {
            ...
            try {
                ...
                ctx.logger?.warn?.(`dsh-plugin-oauth-subs: failed to register /oauth-subs-auth: ${error instanceof Error ? error.message : error}`);
            }
        }
    ...
    ctx.effect(() => {
        ...
        void (async () => {
            try {
                ...
                if (!closed)
                    ctx.logger?.error?.(`dsh-plugin-oauth-subs: failed to start: ${describeError(error)}`);
            }
        })();
        return () => {
            closed = true;
